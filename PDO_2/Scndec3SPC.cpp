#include "Scndec3SPC.h"

// Criar restrições
// ------------------------------------------------
// Funções para montar o lado esquerdo das restrições lineares
CMatrizEsparsa Scndec3SPC::MatrizRestDemanda()	// Monta matriz da restrição de atendimento a demanda
{
	// T * B
	int n_a = n;
...
CMatrizEsparsa Scndec3SPC::MatrizRestDemandaBarraUnica()	// Monta matriz da restrição de atendimento a demanda
{
	int n_a = n;
	n_a = n_a - flag2*sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Scndec3SPC::MatrizLimFluxo()
{
	int n_a = n;
...
CMatrizEsparsa Scndec3SPC::MatrizLimPhgMin()
{
	int n_a = n;
	int JJ = 0;
	for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
...
CMatrizEsparsa Scndec3SPC::MatrizLimPhgMax()
{
	int n_a = n;
	int JJ = 0;
	for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
...
CMatrizEsparsa Scndec3SPC::MatrizBalHid()
{
	// T * R
	int n_a = n;
	double deltaT;
...
CMatrizEsparsa Scndec3SPC::MatrizLimqMin()
{
	int n_a = n;
	int JJ = 0;
	for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
...
CMatrizEsparsa Scndec3SPC::MatrizLimqMax()
{
	int n_a = n;
	int JJ = 0;
	for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
...
CMatrizEsparsa Scndec3SPC::MatrizTup()
{
...
CMatrizEsparsa Scndec3SPC::MatrizTdown()
{
...
CMatrizEsparsa Scndec3SPC::MatrizTupDown()
{
...
CMatrizEsparsa Scndec3SPC::MatrizRampaUp()
{
	// T * I
	if (flag7 == 0)
	{
...
CMatrizEsparsa Scndec3SPC::MatrizRampaDown()
{
	// T * I
	if (flag7 == 0)
	{
...
CMatrizEsparsa Scndec3SPC::MatrizLimPtMin()
{
	if (flag7 == 0)
	{
		// T * I
...
CMatrizEsparsa Scndec3SPC::MatrizLimPtMax()
{
	// T * I
	if (flag7 == 0)
	{
...
CMatrizEsparsa Scndec3SPC::MatrizRestCP()
{
	// T * I
	int n_a = n;
	n_a = n_a - flag2*sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Scndec3SPC::MatrizVmeta()
{
	int n_a = n;
	size_t R = sistema_a->hidreletricasVtr.size();
	int JJ = 0;
...
CMatrizEsparsa Scndec3SPC::MatrizBalPotencia()
{
	int n_a = n;
	n_a = n_a - flag2*sistema_a->hidreletricasVtr.size();
	size_t R = sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Scndec3SPC::MatrizBalVazao()
{
	int n_a = n;
	n_a = n_a - flag2*sistema_a->hidreletricasVtr.size();
	size_t R = sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Scndec3SPC::MatrizFuncProd()
{
	int n_a = n;
	n_a = n_a - flag2*sistema_a->hidreletricasVtr.size();
	size_t R = sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Scndec3SPC::MatrizPhMax()
{
	int n_a = n;
	n_a = n_a - flag2*sistema_a->hidreletricasVtr.size();
	size_t R = sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Scndec3SPC::MatrizReserva()
{
	int n_a = n;
	n_a = n_a - flag2*sistema_a->hidreletricasVtr.size();
	size_t R = sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Scndec3SPC::MatrizCortesF()
{
	// T * I
	int n_a = n;
	n_a = n_a - flag2*sistema_a->hidreletricasVtr.size();
...
vetorfloat2 Scndec3SPC::LimRestDemanda()
{
...
vetorfloat2 Scndec3SPC::LimRestDemandaBarraUnica()
{
	size_t B = sistema_a->barrasVtr.size();
	vetorfloat2 Lim;
...
vetorfloat2 Scndec3SPC::LimFluxo0()
{
	vetorfloat2 Lim;
...
vetorfloat2 Scndec3SPC::LimFluxo2()
{
	vetorfloat2 Lim;
	size_t L = sistema_a->linhasVtr.size();
	size_t B = sistema_a->barrasVtr.size();
...
vetorfloat2 Scndec3SPC::LimPhgMin()
{
	int R = sistema_a->hidreletricasVtr.size();
	int JJ = 0;
	for (int r = 0; r < R; r++)
...
vetorfloat2 Scndec3SPC::LimPhgMax()
{
	int R = sistema_a->hidreletricasVtr.size();
	int JJ = 0;
	for (int r = 0; r < R; r++)
...
vetorfloat2 Scndec3SPC::LimBalHid()
{
	double deltaT;
	int R = sistema_a->hidreletricasVtr.size();
	vetorfloat2 Lim;
...
vetorfloat2 Scndec3SPC::LimQMin()
{
	int R = sistema_a->hidreletricasVtr.size();
	int JJ = 0;
	for (int r = 0; r < R; r++)
...
vetorfloat2 Scndec3SPC::LimQMax()
{
	int R = sistema_a->hidreletricasVtr.size();
	int JJ = 0;
	for (int r = 0; r < R; r++)
...
vetorfloat2 Scndec3SPC::LimTdown()
{
	if (flag7 == 0)
	{
		int n_a = n;
...
vetorfloat2 Scndec3SPC::LimTupDown()
{
...
vetorfloat2 Scndec3SPC::LimRampaUp()
{
	if (flag7 == 0)
	{
		// T * I
...
vetorfloat2 Scndec3SPC::LimRampaDown()
{
	if (flag7 == 0)
	{
		// T * I
...
vetorfloat2 Scndec3SPC::LimPtMin()
{
	if (flag7 == 0)
	{
		// T * I
...
vetorfloat2 Scndec3SPC::LimPtMax()
{
	if (flag7 == 0)
	{
		// T * I
...
vetorfloat2 Scndec3SPC::LimRestCP()
{
	// T * I
	int n_a = n;
	int I = sistema_a->termeletricasVtr.size();
...
vetorfloat2 Scndec3SPC::LimVmeta()
{
	int R = sistema_a->hidreletricasVtr.size();
	vetorfloat2 Lim;
...
vetorfloat2 Scndec3SPC::LimBalPotenciaL()
{
	int R = sistema_a->hidreletricasVtr.size();
	vetorfloat2 Lim;
	DimensionarMatriz(&Lim, R * T, 2);
...
vetorfloat2 Scndec3SPC::LimBalVazaoL()
{
	int R = sistema_a->hidreletricasVtr.size();
	vetorfloat2 Lim;
	DimensionarMatriz(&Lim, R * T, 2);
...
vetorfloat2 Scndec3SPC::LimFuncProdL()
{
	int R = sistema_a->hidreletricasVtr.size();
	int JJ = 0;
	for (int i = 0; i < R; i++)
...
vetorfloat2 Scndec3SPC::LimPhMax()
{
	int R = sistema_a->hidreletricasVtr.size();
	vetorfloat2 Lim;
	DimensionarMatriz(&Lim, R * T, 2);
...
vetorfloat2 Scndec3SPC::LimReserva()
{
	size_t B = sistema_a->barrasVtr.size();
	vetorfloat2 Lim;
...
vetorfloat2 Scndec3SPC::LimCortesF()
{
	// T * I
	int I = sistema_a->termeletricasVtr.size();
	vetorfloat2 Lim;
...
void Scndec3SPC::MatrizRestricoesLineares(CMatrizEsparsa &MM)		// Matriz M é composta de 3 vetores, cada coluna (vetor) é : [indexL indexC Valor]
{
	CMatrizEsparsa M(0);
	if ( sistema_a->GetFlagModeloRede() == 1 )
	{
		M = MatrizRestDemanda(); i_rest_cen[0] = MM.GetNlin();
	...
	{
		M = MatrizRestDemandaBarraUnica(); i_rest_cen[0] = MM.GetNlin();
		MM.JuntarColuna(&M); M.ZerarMatriz();
	...
	M = MatrizLimqMax();
	MM.JuntarColuna(&M); M.ZerarMatriz();
	M = MatrizBalHid(); i_rest_cen[1] = MM.GetNlin();
	MM.JuntarColuna(&M); M.ZerarMatriz();
	M = MatrizTup();									
	MM.JuntarColuna(&M); M.ZerarMatriz();
	M = MatrizTdown();
	...
	M = MatrizReserva(); i_rest_cen[2] = MM.GetNlin();
	MM.JuntarColuna(&M); M.ZerarMatriz();
...
void Scndec3SPC::MatrizLimitesLineares(vetorint * LimTipo, vetorfloat * LimValor)
{
	vetorfloat2 L;
	if ( sistema_a->GetFlagModeloRede() == 1 )
	{
...
Scndec3SPC::Scndec3SPC(CSistema * const sistema_end, const GRBEnv &ambiente_gurobi)
{
	sistema_a = sistema_end;
	cenario = 0;
	reset_stt = 1;
	modeloGRB = new GRBModel(ambiente_gurobi);		// um modelo de otimização para o cenário
	i_rest_cen.resize(3);							// indice das restrições (RHS) que precisam ser atualizadas para cada cenário
	T = sistema_a->GetTt2();
	...
	modeloGRB->getEnv().set(GRB_DoubleParam_IntFeasTol, 1e-2);
...
Scndec3SPC::Scndec3SPC(CSistema * const sistema_end, int cenario_a, Scndec3SPC &cenario0)
{
	sistema_a = sistema_end;
	cenario = cenario_a;
	...
	sol_mipgap_curr = cenario0.sol_mipgap;
	reset_stt = cenario0.reset_stt;
	...
	AtualizarRestricoes();
	//atualizar limites das var. que dependam do periodo e cenário
	AtualizarLUB();
}
Scndec3SPC::~Scndec3SPC(void)
{
	delete modeloGRB;
	delete vars;
}

void Scndec3SPC::CriarVariaveis()
{
	try 
	{
		size_t I = sistema_a->termeletricasVtr.size();
		...
		for (int t = 0; t < T; t++)
		{
			...
			for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//v
				if ( t < sistema_a->GetTt1())
					...
					modeloGRB->addVar(double(sistema_a->hidreletricasVtr[r].GetVmin(t+cenario*(sistema_a->GetTt2()-sistema_a->GetTt1()))), double(sistema_a->hidreletricasVtr[r].GetVmax(t+cenario*(sistema_a->GetTt2()-sistema_a->GetTt1()))), 0.0, GRB_CONTINUOUS, "");
...
void Scndec3SPC::CriarRestricoes()
{
	try
	{
		vetorint LimTipo;
...
void Scndec3SPC::AtualizarRestricoes()
{
	try
	{
	...
	try 
	{
		...
		for (int t = 0; t < T; t++)
		{
			...
			for (size_t r = 0; r < R; r++)
			{
				if ( t < sistema_a->GetTt1())
				{
					vars[delta_x + r].set(GRB_DoubleAttr_LB, double(sistema_a->hidreletricasVtr[r].GetVmin(t)));
					vars[delta_x + r].set(GRB_DoubleAttr_UB, double(sistema_a->hidreletricasVtr[r].GetVmax(t)));
				}
				else
				{
					vars[delta_x + r].set(GRB_DoubleAttr_LB, double(sistema_a->hidreletricasVtr[r].GetVmin(t+cenario*(sistema_a->GetTt2()-sistema_a->GetTt1()))));
					vars[delta_x + r].set(GRB_DoubleAttr_UB, double(sistema_a->hidreletricasVtr[r].GetVmax(t+cenario*(sistema_a->GetTt2()-sistema_a->GetTt1()))));
				}
			}
			delta_x += 3*R;
			if (sistema_a->GetFlagPhmax() == 1)
				delta_x += R;
			for (size_t r = 0; r < R; r++)
				delta_x += 3*sistema_a->hidreletricasVtr[r].GetNGrupos();
			//def
			if ( sistema_a->GetFlagModeloRede() > 0)
			{
				for (size_t b = 0; b < B + 1; b++)	
				{
					...
					for (size_t db = 0; db < sistema_a->barrasVtr[b].demandasPtr.size(); db++)
						cap_d = cap_d + sistema_a->barrasVtr[b].demandasPtr[db]->GetD(cenario, t);
					vars[delta_x + b].set(GRB_DoubleAttr_UB, cap_d);
				}
				delta_x += B + 1;
			}
			else
			{
				...
				for (size_t b = 0; b < sistema_a->barrasVtr.size(); b++)
					for (size_t db = 0; db < sistema_a->barrasVtr[b].demandasPtr.size(); db++)
						cap_d = cap_d + sistema_a->barrasVtr[b].demandasPtr[db]->GetD(cenario, t);
				vars[delta_x].set(GRB_DoubleAttr_UB, cap_d);
				delta_x += 1;
			}
		}
	} 
...
void Scndec3SPC::CriarFuncaoObjetivoRL(const vetorfloat &coefRL)
{
	// Funçao Lagrangiana
	try 
	{
		...
		for (int t = 0; t < T; t++)
		{
			if ((0 <= t) && (t < sistema_a->GetTt1()))		// nós do estágio 1
			{
				...
				for (int i = 0; i < I; i++)
				{
					if (sistema_a->GetFlagInitAproxCT() > 1)
					{
						...
						if (sistema_a->GetFlagTbinaryModel() == 0)
							vars[i + delta].set(GRB_DoubleAttr_Obj, coefRL[i + delta_coef]);				//pt
						else
							vars[i + delta].set(GRB_DoubleAttr_Obj, coefRL[i + delta_coef]);				//pt
						vars[i + I + delta].set(GRB_DoubleAttr_Obj, coefRL[i + I + delta_coef]);			//u
					}
					else
					{
						if (sistema_a->GetFlagTbinaryModel() == 0)
						{
							vars[i + delta].set(GRB_DoubleAttr_Obj, sistema_a->termeletricasVtr[i].GetCoefCustoOper(1)*deltaT + coefRL[i + delta_coef]);	//pt
							vars[i + I + delta].set(GRB_DoubleAttr_Obj, sistema_a->termeletricasVtr[i].GetCoefCustoOper(0)*deltaT + coefRL[i + I + delta_coef]);		//u
						}
						else
						{
							vars[i + delta].set(GRB_DoubleAttr_Obj, sistema_a->termeletricasVtr[i].GetCoefCustoOper(1)*deltaT + coefRL[i + delta_coef]);	//pt
							vars[i + I + delta].set(GRB_DoubleAttr_Obj, (sistema_a->termeletricasVtr[i].GetCoefCustoOper(0) + sistema_a->termeletricasVtr[i].GetCoefCustoOper(1)*sistema_a->termeletricasVtr[i].GetPmin())*deltaT + coefRL[i + I + delta_coef]);		//u
						}
					}
					if (sistema_a->GetFlagTbinaryModel() == 0)
				...
				for (int r = 0; r < R; r++)
				{
					vars[r + I*(3 + flag4 + flag7) + flag1a*(B - 1) + R + delta].set(GRB_DoubleAttr_Obj, coefRL[r + 2*I + delta_coef]);	//v
				}
			}
			else		// nós do estágio 2
			{
				...
				if ((t >= sistema_a->GetTt1()) && ((t + 1 - sistema_a->GetTt1())%(sistema_a->GetTt2() - sistema_a->GetTt1()) == 0))
					if (sistema_a->GetFlagVfol() == true)
						for (int r = 0; r < R; r++)
							vars[r + I*(3 + flag4 + flag7) + flag1a*(B - 1) + (4+flag3)*R + 3*JJ + flag1d*B + (1 - flag1d) + delta].set(GRB_DoubleAttr_Obj, sistema_a->GetCustoVfol()*deltaT);		//vfol
			}
			delta += nt;
			delta_coef += 2*I + R;
		}
	...
	{
			cout << "Exception during Scndec3SPC::CriarFuncaoObjetivoRL()" << endl;	
	}
}
void Scndec3SPC::FixarCondIniciais()
{
...
int Scndec3SPC::ResolverProblemaRL(Scndec3Results * resultadoGurobi, const vetorfloat2 &lambda, const int iter)
{
	...
	try
	{
		...
		if (reset_stt)
			modeloGRB->reset();
		// deve-se resetar caso contrário as soluções ficam MUITO ruins!!

		//ostringstream n_sub;
		//n_sub << cenario;
		//modeloGRB->write("SPC_subproblem" + n_sub.str() + ".lp" );	
