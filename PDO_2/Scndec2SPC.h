...
public:
	...
	Scndec2SPC(CSistema * const sistema_end, const GRBEnv &ambiente_gurobi);
	Scndec2SPC(CSistema * const sistema_end, int cenario_a, Scndec2SPC &cenario0);
	~Scndec2SPC(void);

	int ResolverProblemaRL(Scndec2Results * resultadoGurobi, const vetorfloat2 &lambda, const int iter);
	...
	void SetMaxMipgap(double mipgap_a) {modeloGRB->getEnv().set(GRB_DoubleParam_MIPGap, mipgap_a); modeloGRB->update(); cout << "Precisao setada para: " << mipgap_a << endl;}
};