...
class Scndec1SPC
{
	...
	void MatrizRestricoesLineares(vetorint * indexL, vetorint * indexC, vetorfloat * indexV, vetorint * nnZ, int * n_restricoes);
...
public:
	...
	Scndec1SPC(CSistema * const sistema_end, const GRBEnv &ambiente_gurobi);
	Scndec1SPC(CSistema * const sistema_end, int cenario_a, Scndec1SPC &cenario0);
	~Scndec1SPC(void);

	int ResolverProblemaRL(Scndec1Results * resultadoGurobi, const vetorfloat2 &lambda, const int iter);
	double GetFO() { return fo;}
	int GetNVarPorComp() {return n;}		// comp de cada tipo de subproblema
	double GetMipgap(){return sol_mipgap;}
	int GetStatus() {return nStatus;}
	void SetMaxMipgap(double mipgap_a) {modeloGRB->getEnv().set(GRB_DoubleParam_MIPGap, mipgap_a); modeloGRB->update(); cout << "Precisao do Sub" << cenario << " setada para: " << mipgap_a << endl;}
};