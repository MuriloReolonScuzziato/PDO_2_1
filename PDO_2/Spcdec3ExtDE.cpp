...
CMatrizEsparsa Spcdec3ExtDE::MatrizRestDemanda()	// Monta matriz da restrição de atendimento a demanda
{
	// T * B
	int n_a = no;
...
CMatrizEsparsa Spcdec3ExtDE::MatrizRestDemandaBarraUnica()	// Monta matriz da restrição de atendimento a demanda
{
	int n_a = no;
	n_a = n_a - flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Spcdec3ExtDE::MatrizLimFluxo()
{
	int n_a = no;
...
CMatrizEsparsa Spcdec3ExtDE::MatrizLimPhgMin()
{
	int n_a = no;
	int JJ = 0;
...
CMatrizEsparsa Spcdec3ExtDE::MatrizLimPhgMax()
{
	int n_a = no;
	int JJ = 0;
	for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
...
CMatrizEsparsa Spcdec3ExtDE::MatrizBalHid()
{
	int n_a = no;
	double deltaT;
	int tempo_viagem;
	...
	for (int t = 0; t < T; t++)
	{
		...
		{
			Alin.InserirMatriz(l,c + sistema_a->termeletricasVtr.size() + R,l + R - 1,c + sistema_a->termeletricasVtr.size() + 2*R - 1, &Av, 0, 0);
			for (int tt = 0; tt <= t; tt++)
				Alin.InserirMatriz(l, no + tt * (na / T) + sistema_a->termeletricasVtr.size() + 2*R, l + R - 1, no + tt*(na / T) + sistema_a->termeletricasVtr.size() + 3*R - 1, &Ad, t*R, tt*R);
			if (t > 0)
				Alin.InserirMatriz(l, c - (na / T) + sistema_a->termeletricasVtr.size() + R, l + R - 1, c - (na / T) + sistema_a->termeletricasVtr.size() + 2*R - 1, &AvNeg, 0, 0);
		}
		else
		{
			cen = (t - sistema_a->GetTt1()) / (sistema_a->GetTt2() - sistema_a->GetTt1());
			...
			for (int tt = 0; tt <= t; tt++)
			{
				Alin.InserirMatriz(l,cc + sistema_a->termeletricasVtr.size() + 2*R,l + R - 1,cc + sistema_a->termeletricasVtr.size() + 3*R - 1, &Ad, t*R, tt*R);
				cc += (na / T);
			}
			if (t > 0)
				if (t - cen*(sistema_a->GetTt2() - sistema_a->GetTt1()) == sistema_a->GetTt1())
					Alin.InserirMatriz(l, no + (na / T)*sistema_a->GetTt1() - (na / T) + sistema_a->termeletricasVtr.size() + R,l + R - 1, no + (na / T)*sistema_a->GetTt1() - (na / T) + sistema_a->termeletricasVtr.size() + 2*R - 1, &AvNeg, 0, 0);
				else
					Alin.InserirMatriz(l,c - (na / T) + sistema_a->termeletricasVtr.size() + R,l + R - 1,c - (na / T) + sistema_a->termeletricasVtr.size() + 2*R - 1, &AvNeg, 0, 0);
		}
		l = l + R;
...
CMatrizEsparsa Spcdec3ExtDE::MatrizLimqMin()
{
	int n_a = no;
	int JJ = 0;
	for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
...
CMatrizEsparsa Spcdec3ExtDE::MatrizLimqMax()
{
	int n_a = no;
	int JJ = 0;
	for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
...
CMatrizEsparsa Spcdec3ExtDE::MatrizTup()
{
	if (flag7 == 0)
	{
		// T * sum(Tup)
...
CMatrizEsparsa Spcdec3ExtDE::MatrizTdown()
{
	if (flag7 == 0)
	{
		// T * sum(Tdown)
...
CMatrizEsparsa Spcdec3ExtDE::MatrizTupDown()
{
...
CMatrizEsparsa Spcdec3ExtDE::MatrizRampaUp()
{
	// T * I
	if (flag7 == 0)
	{
...
CMatrizEsparsa Spcdec3ExtDE::MatrizRampaDown()
{
	// T * I
	if (flag7 == 0)
	{
...
CMatrizEsparsa Spcdec3ExtDE::MatrizLimPtMin()
{
	if (flag7 == 0)
	{
		// T * I
...
CMatrizEsparsa Spcdec3ExtDE::MatrizLimPtMax()
{
	// T * I
	if (flag7 == 0)
	{
...
CMatrizEsparsa Spcdec3ExtDE::MatrizRestCP()
{
	// T * I
	int n_a = no;
	n_a = n_a - flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Spcdec3ExtDE::MatrizVmeta()
{
	int n_a = no;
	size_t R = sistema_a->hidreletricasVtr.size();
	int JJ = 0;
	...
	ca = no + sistema_a->termeletricasVtr.size() + R + (sistema_a->GetTt2() - 1)*(na / T);
	for (int cen = 0; cen < sistema_a->GetNCenarios(); cen++)
	{
		for (size_t r = 0; r < R; r++)
		{
			a.RemoverTodosElementos();
			a.InserirElemento(0, ca + r, 1);
		...
		ca += (sistema_a->GetTt2() - sistema_a->GetTt1())*(na / T);
	}
	return Alin;
}
CMatrizEsparsa Spcdec3ExtDE::MatrizBalPotencia()
{
	int n_a = no;
	n_a = n_a - flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size();
	size_t R = sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Spcdec3ExtDE::MatrizBalVazao()
{
	int n_a = no;
	n_a = n_a - flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size();
	size_t R = sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Spcdec3ExtDE::MatrizFuncProd()
{
	int n_a = no;
	n_a = n_a - flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size();
	size_t R = sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Spcdec3ExtDE::MatrizPhMax()
{
	int n_a = no;
	n_a = n_a - flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size();
	size_t R = sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Spcdec3ExtDE::MatrizReserva()
{
	int n_a = no;
	n_a = n_a - flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size();
	size_t R = sistema_a->hidreletricasVtr.size();
	...
	for (int t = 0; t < T; t++)
	{
		...
		for (size_t r = 0; r < R; r++)
		{
			...
			//{
				a.InserirElemento(0, dd + r, 1);
				a.InserirElemento(0, dd - 3*R + r, - 1);
			//}
			//else
			//{
...
CMatrizEsparsa Spcdec3ExtDE::MatrizCortesF()
{
	// T * I
	int n_a = no;
	n_a = n_a - flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Spcdec3ExtDE::MatrizRestAux()
{
	// x - xa = 0
	int n_a = no;
	n_a = n_a - flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size();
	...
	cc = 0;
	cca = no;
	cprecod = sistema_a->termeletricasVtr.size() + (3+flag3)*sistema_a->hidreletricasVtr.size();
	...
	for (int t = 0; t < T; t++)
	{
		...
		{
			a.RemoverTodosElementos();
			a.InserirElemento(0, cc + (3+flag4+flag7)*I + B + R + r, sistema_a->GetPrecondidioner(r + I + R + t*cprecod));	// v
			a.InserirElemento(0, cca + I + R + r, - sistema_a->GetPrecondidioner(r + I + R + t*cprecod));					// va
			Alin.JuntarColuna(&a);
		}
		for (size_t r = 0; r < R; r++)
		{
			a.RemoverTodosElementos();
			a.InserirElemento(0, cc + (3+flag4+flag7)*I + B + 2*R + r, sistema_a->GetPrecondidioner(r + I + 2*R + t*cprecod));	// d
			a.InserirElemento(0, cca + I + 2*R + r, - sistema_a->GetPrecondidioner(r + I + 2*R + t*cprecod));					// da
			Alin.JuntarColuna(&a);
		}
		if (sistema_a->GetFlagPhmax() == 1)
		{
			...
			{
				a.RemoverTodosElementos();
				a.InserirElemento(0, cc + (3+flag4+flag7)*I + B + 4*R + r, sistema_a->GetPrecondidioner(r + I + 3*R + t*cprecod));	// phmax
				a.InserirElemento(0, cca + I + 3*R + r, - sistema_a->GetPrecondidioner(r + I + 3*R + t*cprecod));					// phmaxa
				Alin.JuntarColuna(&a);
			}
		}
		else
		{
			...
			{
				// teria que ter o precontioner aqui tb... e nas depois implementações dessa restrição!!!

				a.InserirElemento(0, ddd + r, - sistema_a->GetPrecondidioner(I + 3*R + t*cprecod));
				for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)
					a.InserirElemento(0, jj + j, + sistema_a->hidreletricasVtr[r].grupoVtr[j].GetPmax() * sistema_a->hidreletricasVtr[r].grupoVtr[j].GetNUnidades() * sistema_a->GetPrecondidioner(I + 3*R + t*cprecod));
				jj += sistema_a->hidreletricasVtr[r].GetNGrupos();
			}
			Alin.JuntarColuna(&a);
		}
...
vetorfloat2 Spcdec3ExtDE::LimRestDemanda()
{
...
vetorfloat2 Spcdec3ExtDE::LimRestDemandaBarraUnica()
{
	size_t B = sistema_a->barrasVtr.size();
	vetorfloat2 Lim;
...
vetorfloat2 Spcdec3ExtDE::LimFluxo0()
{
	vetorfloat2 Lim;
...
vetorfloat2 Spcdec3ExtDE::LimFluxo2()
{
	vetorfloat2 Lim;
	size_t L = sistema_a->linhasVtr.size();
	size_t B = sistema_a->barrasVtr.size();
...
vetorfloat2 Spcdec3ExtDE::LimPhgMin()
{
	int R = sistema_a->hidreletricasVtr.size();
	int JJ = 0;
	for (int r = 0; r < R; r++)
...
vetorfloat2 Spcdec3ExtDE::LimPhgMax()
{
	int R = sistema_a->hidreletricasVtr.size();
	int JJ = 0;
	for (int r = 0; r < R; r++)
...
vetorfloat2 Spcdec3ExtDE::LimBalHid()
{
	double deltaT;
	int cenario;
	int periodo;
...
vetorfloat2 Spcdec3ExtDE::LimQMin()
{
	int R = sistema_a->hidreletricasVtr.size();
	int JJ = 0;
	for (int r = 0; r < R; r++)
...
vetorfloat2 Spcdec3ExtDE::LimQMax()
{
	int R = sistema_a->hidreletricasVtr.size();
	int JJ = 0;
	for (int r = 0; r < R; r++)
...
vetorfloat2 Spcdec3ExtDE::LimTup()
{
	if (flag7 == 0)
	{
...
vetorfloat2 Spcdec3ExtDE::LimTdown()
{
	if (flag7 == 0)
	{
		int I = sistema_a->termeletricasVtr.size();
...
vetorfloat2 Spcdec3ExtDE::LimTupDown()
{
	// T * I	(nos primeiros periodos tem-se linhas com 0, pois não são consideradas determinadas restrições para t < 2 ou t < Tup + 2 ou t < Tdown + 2
	size_t I = sistema_a->termeletricasVtr.size();
	vetorfloat2 Lim;
...
vetorfloat2 Spcdec3ExtDE::LimRampaUp()
{
	if (flag7 == 0)
	{
		// T * I
...
vetorfloat2 Spcdec3ExtDE::LimRampaDown()
{
	if (flag7 == 0)
	{
		// T * I
...
vetorfloat2 Spcdec3ExtDE::LimPtMin()
{
	if (flag7 == 0)
	{
		// T * I
...
vetorfloat2 Spcdec3ExtDE::LimPtMax()
{
	if (flag7 == 0)
	{
		// T * I
...
vetorfloat2 Spcdec3ExtDE::LimRestCP()
{
	// T * I
	int I = sistema_a->termeletricasVtr.size();
	vetorfloat2 Lim;
...
vetorfloat2 Spcdec3ExtDE::LimVmeta()
{
	int R = sistema_a->hidreletricasVtr.size();
	vetorfloat2 Lim;
...
vetorfloat2 Spcdec3ExtDE::LimBalPotenciaL()
{
	int R = sistema_a->hidreletricasVtr.size();
	vetorfloat2 Lim;
	DimensionarMatriz(&Lim, R * T, 2);
...
vetorfloat2 Spcdec3ExtDE::LimBalVazaoL()
{
	int R = sistema_a->hidreletricasVtr.size();
	vetorfloat2 Lim;
	DimensionarMatriz(&Lim, R * T, 2);
...
vetorfloat2 Spcdec3ExtDE::LimFuncProdL()
{
	int R = sistema_a->hidreletricasVtr.size();
	int JJ = 0;
	for (int i = 0; i < R; i++)
...
vetorfloat2 Spcdec3ExtDE::LimPhMax()
{
	int R = sistema_a->hidreletricasVtr.size();
	vetorfloat2 Lim;
	DimensionarMatriz(&Lim, R * T, 2);
...
vetorfloat2 Spcdec3ExtDE::LimReserva()
{
	size_t B = sistema_a->barrasVtr.size();
	vetorfloat2 Lim;
...
vetorfloat2 Spcdec3ExtDE::LimCortesF()
{
	// T * I
	int I = sistema_a->termeletricasVtr.size();
	vetorfloat2 Lim;
...
vetorfloat2 Spcdec3ExtDE::LimRestAux()
{
	int R = sistema_a->hidreletricasVtr.size();
	int I = sistema_a->termeletricasVtr.size();
	vetorfloat2 Lim;
	DimensionarMatriz(&Lim, T * (I + (3+flag3)*R + (1-flag3)), 2);
	if (sistema_a->GetFlagPhmax() == 1)
	{
		IniciaMatriz(&Lim, 0);
		for (size_t i = 0; i < Lim.size(); i++)
...
void Spcdec3ExtDE::MatrizRestricoesLineares(CMatrizEsparsa &MM)		// Matriz M é composta de 3 vetores, cada coluna (vetor) é : [indexL indexC Valor]
{
	CMatrizEsparsa M(0);
...
void Spcdec3ExtDE::MatrizLimitesLineares(vetorint * LimTipo, vetorfloat * LimValor)
{
	vetorfloat2 L;
	if ( sistema_a->GetFlagModeloRede() == 1 )
	{
...
Spcdec3ExtDE::Spcdec3ExtDE(CSistema * const sistema_end) :ambGRB(GRBEnv()),modeloGRB(GRBModel(ambGRB))
{
...
Spcdec3ExtDE::~Spcdec3ExtDE(void)
{
	delete vars;
	delete constr;
}

void Spcdec3ExtDE::CriarVariaveis()
{
	try 
	{
		size_t I = sistema_a->termeletricasVtr.size();
		...
		for (int t = 0; t < T1; t++)
		{
			...
			for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//va
				modeloGRB.addVar(double(sistema_a->hidreletricasVtr[r].GetVmin(t)), double(sistema_a->hidreletricasVtr[r].GetVmax(t)), 0.0, GRB_CONTINUOUS, "");
			for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//da
			{
				double qhmax = 0;
				for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)
					qhmax = qhmax + double (sistema_a->hidreletricasVtr[r].grupoVtr[j].GetQmax() * sistema_a->hidreletricasVtr[r].grupoVtr[j].GetNUnidades());
...
void Spcdec3ExtDE::CriarRestricoes()
{
	try
	{
		vetorint LimTipo;
...
void Spcdec3ExtDE::CriarFuncaoObjetivo()
{
	try 
	{
		GRBQuadExpr fo;
			...
			else
			{
				...
				for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
				{
					if (sistema_a->GetFlagVfol() == true)
						if (((t + 1 - sistema_a->GetTt1())%(sistema_a->GetTt2() - sistema_a->GetTt1()) == 0) && ((t + 1 - sistema_a->GetTt1()) > 0))
						{
							coeficiente = sistema_a->GetCustoVfol() * deltaT * sistema_a->hidreletricasVtr[0].GetProbAfluencia(cen);		//vfol
	...
	{
			cout << "Exception during CriarFuncaoObjetivo" << endl;	
	}
}
void Spcdec3ExtDE::CriarFuncaoObjetivo2()
{
	try 
	{
		GRBQuadExpr fo;
	...
	{
			cout << "Exception during CriarFuncaoObjetivo2" << endl;	
	}
}
void Spcdec3ExtDE::FixarCondIniciais()
{
	int n_a = no - flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size();
	size_t I = sistema_a->termeletricasVtr.size();
	int c = 0;
...
double Spcdec3ExtDE::ResolverProblema(LMRow lambda, Results * results)
{
	try
	{
		...
		//modeloGRB.getEnv().set(GRB_StringParam_LogFile, "logExtDE.txt");		// Escrever log em arquivo
		...
		modeloGRB.getEnv().set(GRB_DoubleParam_TimeLimit, 360);		// Limita o tempo de resolução do problema
		...
		//modeloGRB.getEnv().set(GRB_DoubleParam_FeasibilityTol, 1e-6);
		//modeloGRB.getEnv().set(GRB_DoubleParam_OptimalityTol, 1e-6);	// Define tolerancia de otimalidade
		//modeloGRB.reset();

		// Otimizar
		modeloGRB.optimize();

		if (modeloGRB.get(GRB_IntAttr_Status) == 13)
		...
		if ((nStatus == 2) || (nStatus == 9) || (nStatus == 13))
		{
			...
			{	
				...
				//lambda[i - rest_o] = 0;		// caso queira usar L = o como inicial
				//cout << i << " : " << lambda[i - rest_o] << endl;
				//lambda->push_back(double(constr[i].get(GRB_DoubleAttr_Pi)));
			}
			// Gravar solução x, xa e multiplicadores para os cortes da Heuristica
			// Gravar todos multiplicadores, pois o que define quais é o numero de subproblemas na Heuristica
		...
		cout << "Exception during ResolverProblema()" << endl;
	}
