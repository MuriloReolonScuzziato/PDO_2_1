...
CMatrizEsparsa CProblema_ED_ext::MatrizRestDemanda()	// Monta matriz da restrição de atendimento a demanda
{
	// T * B
	int n_a = no;
...
CMatrizEsparsa CProblema_ED_ext::MatrizRestDemandaBarraUnica()	// Monta matriz da restrição de atendimento a demanda
{
	int n_a = no;
	n_a = n_a - flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size();
	...
	for (size_t i = 0; i < sistema_a->hidreletricasVtr.size(); i++)
		JJ = JJ + sistema_a->hidreletricasVtr[i].GetNGrupos();
	CMatrizEsparsa Alin(0, n + flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size());
	CMatrizEsparsa a(1, n + flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size());
	int c = 0;
	int ca = no;
	for ( int t = 0; t < T; t++)
	{
...
CMatrizEsparsa CProblema_ED_ext::MatrizVmeta()
{
	...
	//		ca = no + r + sistema_a->termeletricasVtr.size() + R;
	//		ca += (T - 1)*(na / T) - cen * (sistema_a->GetTt2() - sistema_a->GetTt1())*(na / T);
	//		a.RemoverTodosElementos();
	//		a.InserirElemento(0, ca, 1);
	//		a.InserirElemento(0, c + 4*R + 3*JJ + flag1*(sistema_a->barrasVtr.size()) + (1 - flag1), 1);
	//		Alin.JuntarColuna(&a);
	//	}
	//}

	c = (3+flag4+flag7)*sistema_a->termeletricasVtr.size() + flag1*(sistema_a->barrasVtr.size() - 1) + R + (sistema_a->GetTt2() - 1)*(n_a / T);
	...
	for (int cen = 0; cen < sistema_a->GetNCenarios(); cen++)
	{
		for (size_t r = 0; r < R; r++)
		{
			...
			if (sistema_a->GetFlagVfol() == true)
				a.InserirElemento(0, c + r + 4*R + 3*JJ + flag1*(sistema_a->barrasVtr.size()) + (1 - flag1), 1);
			Alin.JuntarColuna(&a);
		}
	...
	for (int t = 0; t < T; t++)
	{
		...
		{	
			for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)
			{
				...
				a.InserirElemento(0, jj + j + JJ, sistema_a->hidreletricasVtr[r].grupoVtr[j].GetPmin());				// z -> adiciona um termo (1 - z)*Pmin na função, para phg <= pmin qdo z = 0; o rhs estava negativo, devido a aproximação linear, fazendo com q a hidro ficasse obrigatoriamente ligada
...
CMatrizEsparsa CProblema_ED_ext::MatrizRestAux()
{
	...
	cc = 0;
	cca = no;
	cprecod = sistema_a->termeletricasVtr.size() + 4*sistema_a->hidreletricasVtr.size();
	for (int t = 0; t < T; t++)
	{
		if (((t + 1 - sistema_a->GetTt1())%(sistema_a->GetTt2() - sistema_a->GetTt1()) == 0) && ((t + 1 - sistema_a->GetTt1()) > 0))
			nt = (n_a / T) + flag2*sistema_a->hidreletricasVtr.size();
	...
	for (int t = 0; t < T; t++)
		for (int r = 0; r < R; r++)
		{
			for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)
			{
				...
				Lim[jj + j][1] = sistema_a->hidreletricasVtr[r].grupoVtr[j].CoefPhg() + sistema_a->hidreletricasVtr[r].grupoVtr[j].GetPmin();
...
vetorfloat2 CProblema_ED_ext::LimRestAux()
{
	...
	int I = sistema_a->termeletricasVtr.size();
	vetorfloat2 Lim;
	DimensionarMatriz(&Lim, T * (I + 4*R), 2);
	IniciaMatriz(&Lim, 0);
	for (size_t i = 0; i < Lim.size(); i++)
		Lim[i][0] = 1;
	return Lim;
...
void CProblema_ED_ext::MatrizRestricoesLineares(vetorint * indexL, vetorint * indexC, vetorfloat * indexV, vetorint * nnZ, int * n_restricoes)		// Matriz M é composta de 3 vetores, cada coluna (vetor) é : [indexL indexC Valor]
{
	CMatrizEsparsa M(0);
	int count = 0;
...
CProblema_ED_ext::CProblema_ED_ext(CSistema * const sistema_end) :ambGRB(GRBEnv()),modeloGRB(GRBModel(ambGRB))
{
	...
	for (size_t i = 0; i < sistema_a->hidreletricasVtr.size(); i++)
		JJ = JJ + sistema_a->hidreletricasVtr[i].GetNGrupos();
	no = T * ((3+flag4+flag7)*sistema_a->termeletricasVtr.size() + sistema_a->barrasVtr.size() - 1 + 5*sistema_a->hidreletricasVtr.size() + 3*JJ + sistema_a->barrasVtr.size()) + flag2 * sistema_a->hidreletricasVtr.size() * sistema_a->GetNCenarios();
...
CProblema_ED_ext::~CProblema_ED_ext(void)
{
	delete vars;
	delete constr;
}

void CProblema_ED_ext::CriarVariaveis()
{
	try 
	{
		size_t I = sistema_a->termeletricasVtr.size();
...
void CProblema_ED_ext::CriarRestricoes()
{
	try
	{
		int n_restricoes = 0;
...
void CProblema_ED_ext::CriarFuncaoObjetivo()
{
	try 
	{
		GRBQuadExpr fo;
...
void CProblema_ED_ext::CriarFuncaoObjetivo2()
{
	try 
	{
		GRBQuadExpr fo;
...
void CProblema_ED_ext::FixarCondIniciais()
{
	int n_a = no - flag2*sistema_a->GetNCenarios()*sistema_a->hidreletricasVtr.size();
	size_t I = sistema_a->termeletricasVtr.size();
	int c = 0;
...
double CProblema_ED_ext::ResolverProblema(LMRow lambda)
{
	try
	{
