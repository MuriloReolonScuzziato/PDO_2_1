#include "Spcdec1SPD.h"

// Criar restrições
// ------------------------------------------------

// Funções para montar o lado esquerdo das restrições lineares
CMatrizEsparsa Spcdec1SPD::MatrizRestDemanda(int n_a, int no)	// Monta matriz da restrição de atendimento a demanda
{
	...
	int c = sistema_a->termeletricasVtr.size() + sistema_a->barrasVtr.size() - 1 + sistema_a->hidreletricasVtr.size();
...
CMatrizEsparsa Spcdec1SPD::MatrizRestDemandaBarraUnica(int n_a, int no)	// Monta matriz da restrição de atendimento a demanda
{
	...
	if ( sistema_a->GetFlagModeloRede() == 0)
		a.InserirElemento(0, sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size(), 1);		// def
	else // flag1 == 2
		for (size_t b = 0; b < sistema_a->barrasVtr.size(); b++)		// def
			a.InserirElemento(0, b + sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size(), 1);		// def
	Alin.JuntarColuna(&a);
	
	return Alin;
}
CMatrizEsparsa Spcdec1SPD::MatrizLimFluxo(int n_a, int no)
{
	...
	else // flag1 == 2
	{
		...
		Alin.InserirMatriz(0, sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size(), sistema_a->linhasVtr.size() - 1, sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size() + sistema_a->barrasVtr.size() - 1, Adef, 0, 0);
	}
	return Alin;
}
...
vetorfloat2 Spcdec1SPD::LimRestDemanda(int n_a, int no)
{
	int cen = 0;
	int t = 0;
...
vetorfloat2 Spcdec1SPD::LimRestDemandaBarraUnica(int n_a, int no)
{
	int cen = 0;
	int t = 0;
	if (no < sistema_a->GetTt1())		// identifica o cenário e o período de tempo do nó
...
vetorfloat2 Spcdec1SPD::LimFluxo0(int n_a, int no)
{
...
vetorfloat2 Spcdec1SPD::LimFluxo2(int n_a, int no)
{
	int L = sistema_a->linhasVtr.size();
	vetorfloat2 Lim;
	DimensionarMatriz(&Lim, L, 2);
...
void Spcdec1SPD::MatrizRestricoesLineares(int n_a, CMatrizEsparsa &MM, int no)		// Matriz M é composta de 3 vetores, cada coluna (vetor) é : [indexL indexC Valor]
{
	CMatrizEsparsa M(0);
	if ( sistema_a->GetFlagModeloRede() == 1 )
	{
...
void Spcdec1SPD::MatrizLimitesLineares(int n_a, vetorint * LimTipo, vetorfloat * LimValor, int no)
{
	vetorfloat2 L;
	if ( sistema_a->GetFlagModeloRede() == 1 )
	{
...
Spcdec1SPD::Spcdec1SPD(CSistema * const sistema_end, const GRBEnv &ambiente_gurobi)
{
	...
	for (int nno = 0; nno < n_nos; nno++)
	{
		n[nno] = sistema_a->termeletricasVtr.size() + sistema_a->barrasVtr.size() - 1 + sistema_a->hidreletricasVtr.size() + sistema_a->barrasVtr.size();
...
void Spcdec1SPD::CriarVariaveis(int no)
{
	try 
	{
...
void Spcdec1SPD::CriarRestricoes(int no)
{
	try
	{
		vetorint LimTipo;
...
void Spcdec1SPD::CriarFuncaoObjetivoRL(const vetorfloat * const lambda, int no)
{
	try 
	{
		int cen = 0;
		...
		// Deficit
		delta = sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1) + sistema_a->hidreletricasVtr.size();
...
int Spcdec1SPD::ResolverProblemaRL(Spcdec1Results * resultadoGurobi, const vetorfloat * const lambda, const int iter, int no)
{
	...
	try
	{
		...
		resultadoGurobi->GravarSolucao(fo_n, x_n, nStatus, 1 + resultadoGurobi->GetI() + no);
	} catch(GRBException e) {
		cout << "Error code = " << e.getErrorCode() << endl;
		cout << e.getMessage() << endl;
		resultadoGurobi->GravarSolucao(fo_n, x_n, e.getErrorCode(), 1 + resultadoGurobi->GetI() + no);
	} catch(...) {
		cout << "Exception during optimization" << endl;
	}
	return 0;
...
Spcdec1SPD::Spcdec1SPD(CSistema * const sistema_end)
{
	sistema_a = sistema_end;
	n_nos = sistema_a->GetTt1() + (sistema_a->GetTt2() - sistema_a->GetTt1()) * sistema_a->GetNCenarios();
	n.resize(n_nos);
...
void Spcdec1SPD::GerarRestricoes(int no)
{
	Mind[no].resize(0);Mval[no].resize(0);Mbeg[no].resize(0);dtipo[no].resize(0);dvalor[no].resize(0);
	CMatrizEsparsa MM(0, n[no]);
	MatrizRestricoesLineares(n[no], MM, no);
	MM.SparseMatriz(Mval[no], Mind[no], Mbeg[no]);
	MatrizLimitesLineares(n[no], &dtipo[no], &dvalor[no], no);
	Nrestricoes[no] = MM.GetNlin();
}
void Spcdec1SPD::GerarVarBounds(int no, double *lbd , double *ubd)
{
	// variáveis em cada problema somente para um nó
	int cen = 0;
	int t = 0;
...
void Spcdec1SPD::GerarCoefFuncaoObjetivo(int no, double *cst)
{
	int cen = 0;
	int t = 0;
	if (no < sistema_a->GetTt1())		// identifica o cenário e o período de tempo do nó
...
void Spcdec1SPD::GetBDesc( int comp , int *Bbeg , int *Bind , double *Bval , double *lhs , double *rhs , double *cst , double *lbd , double *ubd )
{
...
int Spcdec1SPD::GetANZ(int comp)
{
	// Retorna o numero de multiplicadores de lagrange considerado em cada subproblema
	return ( sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size() );
}
void Spcdec1SPD::GetADesc( int comp , int *Abeg , int *Aind , double *Aval )
{
	// Termos da RL
	// lambda [pt ph v d phmax]
	// x = [pt teta ph phmax def]
	int nt_dual = sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size();
	int T = sistema_a->GetTt1() + sistema_a->GetNCenarios() * (sistema_a->GetTt2() - sistema_a->GetTt1());
	int ind = 0;
	int iAbeg = 0;
	Abeg[iAbeg++] = 0;
