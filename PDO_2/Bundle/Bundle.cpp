...
#define NOISE_REDUCTION_FIRST 0
...
NDOSolver::NDOStatus Bundle::Solve( void )
{
 ...
 do {
  ...
  if( ( ( tSPar1 & tSP1Msk ) == kHLTTS ) && ( *FiLambda < HpINF ) ) {
   ...
   if( vStar <= tSPar2 * EpsU * AFL ) {
    ...
    if( - vl < Eps<HpNum>() )  // v( t ) is [almost] constant ==> D*_t [~]= 0
     ...
     tt = fmin( tStar , ( tSPar2 * EpsU * AFL * Nearly + vc ) / ( - vl ) );
  ...
  if( EStps ) {
   ...
   EpsCurr = fmax( EpsLin , fmin( EpsCurr , ABS( EInit ) ) );
   }
  else
  ...
  for( ;; ) {   // ... possibly more than once due to precision issues
   ...
   // something wrong with the FiOracle's precision
    ...
    if( ( DSTS >= EpsLin * fmax( HpNum( 1 ) , ABS( *FiLambda ) ) ) &&
	( t < tMaior ) ) {
     t = fmin( t * mxIncr , tMaior );
   ...
   EpsCurr = fmax( EpsLin , fmin( EpsCurr , EpsFi ) * ABS( EDcrs ) );
  ...
  if( SSDone ) {  // SS - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   tt = fmin( fmin( tMaior , t * mxIncr ) , fmax( t * mnIncr , tt ) );
   ...
   tt = fmax( fmax( tMinor , t * mxDecr ) , fmin( t * mnDecr , tt ) );
   ...
   if( CNSCntr < MnNSC )  // decreasing t is inhibited
    ...
    if( *Alfa1 <= m3 * Sigma ) {
     ...
     switch( tSPar1 & tSP1Msk ) {
      case( kSLTTS ):
      case( kHLTTS ):
       if( vStar <= tSPar2 * EpsU * fmax( ABS( *FiLambda ) , HpNum( 1 ) ) ) {
	BLOG( 1 , " small v" );
	tt = t;
        }
  ...
  if( tSPar1 & kEGTTS )  // endgame t-strategy: note the "/ 10"!!
   if( DSTS < EpsLin * fmax( ABS( *FiLambda ) , HpNum( 1 ) ) / 10 ) {
    tt = fmax( t * ( mxDecr + mnDecr ) / 2 , tMinor );
    BLOG( 1 , " ~ endgame, t = " << tt );
    }
...
void Bundle::AddVariables( Index NNwVrs , cLMRow IVs )
{
 ...
 if( IVs ) {  // check if any of the variables is strictly nonzero - - - - - -
  cHpNum MeD = fmax( Master->EpsilonD() , Oracle->GetBndEps() );
  Index i = 0;
...
void Bundle::FormD( void )
{
 ...
 if( *FiLambda < HpINF )
  Master->SetPar( MPSolver::kZero ,
		  EpsLin * fmax( ABS( *FiLambda ) , HpNum( 1 ) )
		          / ( fmax( tStar / t , HpNum( 1 ) ) ) );
 ...
 {        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  {        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   // set time limit- - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   if( MaxTime && NDOt ) {
    if( NDOt->Read() >= MaxTime ) {  // time is over already
     Result = kStpTime;
     break;
     }
  ...
  if( *FiLambda < HpINF )
   EpsU = ( DSTS + Sigma ) / fmax( ABS( *FiLambda ) , HpNum( 1 ) );
  else
  ...
  for( Index i ; ( i = *(MBse++) ) < InINF ; Mlt++ )
   if( *Mlt >= Eps<HpNum>() )
    if( ( *Mlt >= 1 - MPEOpt ) && Master->IsSubG( i ) ) {
     ...
     OOBase[ i ] = fmin( SIndex( -1 ) , OOBase[ i ] );
     }
    else
     if( OOBase[ i ] > 0 )
      OOBase[ i ] = 0;
...
bool Bundle::CheckAlfa( const bool All )
{
 ...
 if( IsEasy ) {
  for( Index k = 0 ; k++ < NrFi ; )
   if( ! IsEasy[ k ] ) 
    DeltaAlfa[ k ] = - fmax( ABS( FiLambda[ k ] ) , HpNum( 1 ) ) * MPEOpt;
  }
 else
  for( Index k = 0 ; k++ < NrFi ; )
   DeltaAlfa[ k ] = - fmax( ABS( FiLambda[ k ] ) , HpNum( 1 ) ) * MPEOpt;
 ...
 if( NNegs ) {  // any negative alfa has been found - - - - - - - - - - - - -
  ...
  for( Index k = 0 ; k++ < NrFi ; ) {
   ...
   if( DAk < - fmax( ABS( FiLambda[ k ] ) , HpNum( 1 ) ) * MPEOpt ) {
...
void Bundle::StrongCheckAlfa( void )
{
  ...
  if( *FiLambda < HpINF ) {  // only if Fi( Lambda ) is defined
   ...
   for( Index i = 0 ; i < Master->MaxName() ; i++ ) {
    ...
    if( WFi < InINF ) {
     cHpNum Ai = Oracle->GetVal( i );
     cHpNum AEps = fmax( ABS( FiLambda[ WFi ] ) , HpNum( 1 ) ) * MPEOpt;
...
void Bundle::CmptaBPX( void )
{
 ...
 aBP4 = fmin( aBP3 , ( BPar4 >= 0 ? Index( ceil( BPar4 ) ) :
