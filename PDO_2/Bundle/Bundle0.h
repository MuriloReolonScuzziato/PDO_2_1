...
/**< Constructor of the class. The parameter `iStrm', if provided, is taken as
   ...
   FiOracle.h]. Then, the above parameters now have the following meaning:
    ...
    "emergency mechanism" reduces it, then it will remain "at least as
    small" in all the following iterations, even if the value computed by
    the "regular mechanism" would be larger. If EInit < 0 instead, then
    each time a "regular step" is computed the precision is reset to that
    dictated by the "regular mechanism", even if it is larger than the
    current one (for instance, if EStps == 0, see below, then the
    precision is reset to ABS( EInit )).
    ...
    precision than EpsLin is not required. That said, the formula is:

     precision = ABS( EInit ) if EStps == 0
    ...
    is the current estimate of the optimality measure, then
