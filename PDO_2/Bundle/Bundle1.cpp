...
void Bundle::SetFiOracle( FiOracle *Fi )
{
 ...
 if( Fi ) {  // setting a new oracle- - - - - - - - - - - - - - - - - - - - -
   ...
   *(LamBase++) = Inf<Index>();  // set a "floor" to the bases: this is used
   *(nBase++) = Inf<Index>();    // in FormD() when they are read backwards

   LamBase[ MaxNumVar + 1 ] = Inf<Index>();  // put a stop to the second part
   nBase[ MaxNumVar + 1 ] = Inf<Index>();    // of the bases, which will hold
                                             // the indices of the variables
                                             // added/removed

   LamBase[ LamDim = 0 ] = Inf<Index>();  // at start no variables are active
  ...
  VectAssign( LowerBound , - Inf<HpNum>() , NrFi + 1 );
  TrueLB = false;

  VectAssign( RfrncFi , HpNum( 0 ) , NrFi + 1 );
  *FiLambda1 = *FiBest = Inf<HpNum>();         // Fi( Lambda ) is not known

  whisG1 = new Index[ NrFi ];
  VectAssign( whisG1 , Index( Inf<Index>() ), NrFi );
  ...
  VectAssign( Alfa1 , HpNum( 0 ) , NrFi + 1 );
   ...
   VectAssign( FiVals , Inf<HpNum>() , NONMONOTONE );
  #endif
 ...
 *FiLambda = *FiBest = Inf<HpNum>();  // Fi( Lambda ) is not known
...
NDOSolver::NDOStatus Bundle::Solve( void )
{
 ...
 do {
  ...
  if( ( ( tSPar1 & tSP1Msk ) == kHLTTS ) && ( *FiLambda < Inf<HpNum>() ) ) {
  ...
  if( *FiLambda < Inf<HpNum>() ) {  // .. but only if Fi( Lambda ) is defined
   if( TrueLB )
  ...
  if( *FiLambda1 == Inf<HpNum>() )
   ...
   if( *FiLambda == Inf<HpNum>() ) {  // if reached feasibility- - - - - - - -
                                      // - - - - - - - - - - - - - - - - - - -
    #if( NONMONOTONE )
...
{
 assert( ( whch && whch[ hwmny ] == Inf<Index>() ) ||
	 ( ( ! whch ) && ( ! hwmny ) ) );
 ...
 if( whch )
  if( PPar2 ) {
   ...
   for( Index h ; ( h = *(tLB++) ) < Inf<Index>() ; tL++ )
    if( h == *tw ) {
     tw++;
...
Bundle::~Bundle()
{
 ...
 BLOG2( 1 , NDOt , endl << "Eps. : " << Oracle->GetVal() );
...
void Bundle::FormD( void )
{
 ...
 {        //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  {        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   if( i == Inf<Index>() )  // there are no *removable* items in Base - - - -
   ...
   if( i == Inf<Index>() ) {  // there are no removable items at all- - - - -
  ...
  VectAssign( whisZ + 1 , Index( Inf<Index>() ) , NrFi );
  // the scalar products have changed
  VectAssign( ScPr1 , HpNum( Inf<HpNum>() ) , NrFi );
   ...
   *(++NewStuff) = Inf<Index>();          // and put termination marks to the
   nBase[ LamDim = nBD ] = Inf<Index>();  // vector of newly added stuff and
                                          // nBase
 ...
 if( ParIter > Index( PPar1 ) ) {  // skip the first PPar1 iterations
  // (all of them if PPar3 == 0 ==> PPar1 == Inf<Index>() )
  ...
  if( ! Master->NumNNVars() )  // there are no NN variables - - - - - - - - -
   for( Index k ; ( k = LamBase[ oBD ] ) < Inf<Index>() ; oBD++ )
  ...
  if( nBD < oBD ) {  // some variables have been eliminated - - - - - - - - -
   BHasChgd = true;                // signal it
   nBase[ LamDim = nBD ] = Inf<Index>();  // and put the termination mark to nBase
...
void Bundle::FormLambda1( HpNum Tau )
{
  ...
  if( Master->NumNNVars() )             // there are NN vars and UB vars
   if( Master->NumNNVars() == NumVar )  // actually, all variables are NN
    if( PPar2 ) {
     ...
     for( Index h ; ( h = *(tLB++) ) < Inf<Index>() ; tL1++ ) {
      if( *tL1 < 0 )
       *tL1 = 0;
 ...
 VectAssign( whisG1 + 1 , Index( Inf<Index>() ) , NrFi );
 ...
 *FiLambda1 = Oracle->Fi( 0 );  // the 0-th component
 ...
 if( *FiLambda1 == Inf<HpNum>() )    // Fi() is not defined in Lambda1
  DeltaFi = Inf<HpNum>();
 else
 ...
 if( *FiLambda1 == - Inf<HpNum>() )  // error in computing Fi()
  return( true );
 ...
 Index wFi = 0;  // which component of Fi the new item shall belong to; it
                 // *can* be 0, as the 0-th component (if Inf<HpNum>()) may
 ...
 for( Index Ftchd = 0 ; Ftchd < aBP3 ; ) {
  ...
  if( FiLambda1[ wFi ] == Inf<HpNum>() )  // it is a constraint
  ...
  if( cp < Inf<Index>() ) {  // the item is a copy- - - - - - - - - - - - - -
   ...
   if( OrigA1k > Alfa1k ) {        // if the copy has smaller Alfa than the
    ...
    wh = Inf<Index>();               // otherwise, nothing new has happened
   }
  else {              // insert the item, if there is space - - - - - - - - -
   if( wh < Inf<Index>() )     // someone has been selected in BStrategy()
    Master->RmvItem( wh );     // remove it from the MP
   else
    wh = FindAPlace( wFi );    // find a spot in the bundle

   if( wh == Inf<Index>() ) {  // no space found ...
  ...
  if( wh < Inf<Index>() ) {
   ...
   if( FiLambda1[ wFi ] < Inf<HpNum>() ) {  // it is a subgradient
    if( ( whisG1[ wFi ] == Inf<Index>() ) || ( Alfa1k < Alfa1[ wFi ] ) ||
 ...
 *ScPr1 = Master->ReadGid();
 ...
 for( wFi = 0 ; wFi++ < NrFi ; )
  if( whisG1[ wFi ] < Inf<Index>() ) {
   if( Alfa1[ wFi ] == Inf<HpNum>() )
    Alfa1[ wFi ] = (Master->ReadLinErr())[ whisG1[ wFi ] ];

   *Alfa1 += Alfa1[ wFi ];

   if( ScPr1[ wFi ] == Inf<HpNum>() )
...
void Bundle::GotoLambda1( void )
{
 ...
 VectAssign( Alfa1 , HpNum( Inf<HpNum>() ) , NrFi + 1 );
...
void Bundle::UpdtCntrs( void )
{
 ...
 if( MBse ) {
  for( Index i ; ( i = *(MBse++) ) < Inf<Index>() ; Mlt++ )
   if( *Mlt >= Eps<HpNum>() )
...
bool Bundle::CheckAlfa( const bool All )
{
 ...
 HpNum mA = 0;  // cumulative error

 for( Index k = 0 ; k++ < NrFi ; ) {
  if( IsEasy && IsEasy[ k ] )  // skip easy components
   continue;

  // define the small epsilon that should be ignored- - - - - - - - - - - - -

  cHpNum AEps = max( ABS( FiLambda[ k ] ) , HpNum( 1 ) ) * MPEOpt;

  // ask the MP Solver to check for negative alfas- - - - - - - - - - - - - -

  cHpNum mAk = Master->CheckLinErr( AEps , All , k );
  mA += mAk;

  if( mAk < 0 ) {  // if they are found - - - - - - - - - - - - - - - - - - -
   //clog << "FiLambda[ " << k << " ] = " << FiLambda[ k ] << " mAk = " << mAk << endl;
   FiLambda[ k ] -= mAk;
   if( ! All )          // if it is Lambda1 to be tested
    Alfa1[ k ] -= mAk;  // Alfa1[ k ] is significative, correct it
   }
  }  // end( for( k ) )

 if( mA < 0 ) {  // any component had a negative alfa - - - - - - - - - - - -
  *FiLambda -= mA;  // Fi( Lambda ) was incorrect
  if( ! All ) {     // if it is Lambda1 to be tested
   *Alfa1 -= mA;    // *Alfa1 is significative, correct it
   DeltaFi -= mA;   // DeltaFi was also wrong
   }
...
{
 #if( CHECK_DS & 1 )
  if( *FiLambda < Inf<HpNum>() ) {  // only if Fi( Lambda ) is defined
 ...
 if( Master ) {
  ...
  if( Master->IsSubG( i ) )  // it is a subgradient
   if( whisG1[ wFi ] == i )  // it is the representative of wFi
    whisG1[ wFi ] = Inf<Index>();   // a new representative is needed
...
inline bool Bundle::FindNextSG( Index &wFi )
{
 ...
 for( Index i = NrFi + 1 ; i-- ; wFi = ( wFi < NrFi ? wFi + 1 : 0 ) ) {
  if( wFi ) {
   ...
   if( *FiLambda1 < Inf<HpNum>() )    // Fi[ 0 ]( Lambda1 ) is defined
...
inline Index Bundle::BStrategy( cIndex wFi )
{
 ...
 // required, so it may return Inf<Index>() and nothing bad may happen

 // in particular, it returns Inf<Index>() is there is plenty of space left
 ...
 if( OOBase[ wh ] < 0 )    // all items are non-removable: nothing else to
  return( Inf<Index>() );  // do (except maybe complaining very loudly)
 else
 ...
 for( Index wFi2 = wFi ; ; ) {
  ...
  if( whisZ[ wFi2 ] == Inf<Index>() ) {
   ...
   if( whZ < Inf<Index>() ) {  // if there is space for Z[ wFi2 ]
  ...
  HpNum tMin = Inf<HpNum>();
  if( MBse ) {
   for( Index h ; ( h = *(MBse++) ) < Inf<Index>() ; Mlt++ )
  ...
  if( wh < Inf<Index>() )  // a place has been found
   break;                  // all done
...
Index Bundle::FindAPlace( cIndex wFi )
{
 ...
 // are no possible positions left, then Inf<Index>() is returned
