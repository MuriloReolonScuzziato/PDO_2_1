...
inline void BMinQuad::CalcOptDir( HpNum ti )
{
 ...
 Index tmpMVA = fmin( MaxVarAdd , B2Dim + MB2Dim );  // max number of varibales
 ...
 do {
  ...
  do {
   ...
   {                    //- - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    cHpNum eDir = eD * fmax( BDim , Index( 1 ) );
	...
	continue;
     ...
     cLMNum dh = ti * di[ h ];
     step = fmin( step , ( dh + bounds[ h ] ) / ( dh + d1h ) );
     }
  ...
  if( Bf < HpINF ) {  // skipping the tests if Bf is undefined
   ...
   cHpNum RqrdDcrs = 
        fmax( MnDcrs , HpEps * fmax( BDim , Index( 1 ) ) * ABS( Bf ) );
   ...
   if( Bf - newBf < - RqrdDcrs ) {  // an increasing step
    ...
    if( Bf - newBf <= RqrdDcrs ) {  // a "short" step
     ...
     if( newBf + eD * ( ABS( newBf ) / SpaceDim ) < BfBot ) {
      ...
      #endif
      tmpMVA = fmin( MaxVarAdd , B2Dim + MB2Dim );  //... the "strict" pricing
