 ...
 * Bundle" algorithms for NonDifferentiable Optimization. Objects of
 * derived classes, actually implementing the solvers, are thought to be
 * used by an algorithm conforming to the NDOSolver/FiOracle abstract
 * interface [see NDOSolver.h and FiOracle.h], such as the Bundle class
 * [see Bundle.h].
 *
 * \version 0.62
 *
 ...
 *         Universita' di Pisa \n
 *
 * Copyright &copy 2002 - 2013 by Antonio Frangioni.
 */
...
/**< The dual optimal solution z* lies in the domain of Fi_{B,Lambda}*; at
   ...
   \note For "easy" components of Fi() [see FiOracle::GetBNC() ...], no
         subgradient of that component are ever produced, so a call to
	 ReadMult( wFi ) with wFi the index of an "easy" component make no
	 sense. In this case, however, something else naturally "takes the
	 place" of the multipliers: the optimal solution x[ wFi ]* of the
	 Master Problem in terms of the "original variables" of the wFi-th
	 component. Thus, calls to ReadMult( wFi ) with an "easy" wFi are
	 indeed allowed, with the following different meaning: the vector
	 returned by the method (that may be "dense" or "sparse" as in the
	 standard case) represents x[ wFi ]*, and it is therefore a
...
/*--------------------------------------------------------------------------*/
   ...
   virtual HpNum CheckLinErr( cHpNum AEps , const bool All = false ,
			      cIndex wFi = Inf<Index>() ) = 0;

/**< Checks the existence of linearization errors that be strictly less than
   - AEps: as AEps is supposed to be >= 0, this means "substantially
   negative" linearization errors. If they are found, the current value of
   Fi is too small; the smallest linearization error (largest in absolute
   value among the negative ones) must be subtracted from the value of Fi,
   thus increasing it.

   When Fi is decomposable, the alfas are partitioned among the components:
   CheckLinErr( ,, wFi ) operates onto the alfas corresponding to component
   wFi if 1 <= wFi <= NrFi, and onto all the alfas otherwise (there is
   implicitly one alfa corresponding to the 0-th component, but it is
   always 0 and it is always exact). For "easy" components of Fi() [see
   FiOracle::GetBNC() ...] linearization errors are never computed, so the
   MPSolver is allowed to assume that calls to CheckLinErr( ... wFi ) with
   wFi an "easy" component will never occur.

   CheckLinErr() must look for this condition and, in case, change the alfas;
   the returned value must be the (negative) number to be subtracted from Fi,
   or 0 if no negative alfas have been found.

   If All == false, only the "newly inserted" subgradients need to be checked;
   that is, all those inserted *after* the previous call to CheckLinErr() or
   ChangeCurrPoint() [any version, see below]. Otherwise, all the subgradients
   in the bundle need to be checked. Of course, if 1 <= wFi <= NrFi then only
   the newly inserted subgradients corresponding to component wFi have to be
   checked. */
...
/**< When the correct values have been written in the vector SG returned by
   GetItem() [see above], SetItemBse() should be used to tell the MPSolver the
   "format" of the information. If SGBse == 0, then SG is in "dense" format,
   i.e., SG[ i ] contains the entry of the subgradient relative to the variable
   with name `i', i = 0 ..  CrrSGLen - 1. If SGBse != 0 instead, then SG is
   in "sparse" format, i.e., SG[ i ] is the entry of the subgradient relative 
   to the variable with name `SGBse[ i ]', i = 0 ..  SGBDm - 1. SGBse is
   assumed to be ordered in increasing sense and Inf<Index>()-terminated, i.e.,
   SGBse[ SGBDm ] == Inf<Index>(); if SGBse == 0, then the value of SGBDm is
   ignored. If SetItemBse() is *not* called, then the "dense" format is
   assumed.
...
/**< After that SetItemBse() has been called, CheckSubG() (if the item is
   ...
   subgradients is finite). Also, the scalar product between the primal
   optimal solution d* of the MP and SG must be calculated and returned in
   ScPri.
...
/**< After that SetItemBse() has been called, CheckCnst() [if the item is a
   ...
   identical (element-wise) to GS, if such an item exists, or Inf<Index>()
   otherwise. This allows to check for useless copies (but this check is
   costly and can be deactivated, see CheckIdentical() above). Also, the
   scalar product between the primal optimal solution d* of the MP and SG
   must be calculated and returned in ScPri.
...
/**< Adds `AdDm' new variables to the current active set, whose names are
   ...
   to be found *after* all the variables that were present previously.
   In this case, the caller is authorized to use the old pointer as the new
   pointer (and checking this is exactly how the MPSolver detects the fact),
   overwriting the "Inf<Index>()" which previously terminated the active set with
   the names of the variables to be added *before* the call to AddActvSt()
   (i.e., Addd = AVrs +  <n. of variables in the old active set>).
...
/*--------------------------------------------------------------------------*/
   ...
   virtual void ChgAlfa( cHpRow NewAlfa , cIndex wFi = Inf<Index>() ) = 0;
...
/**< Changes the vector of the linearization errors of the subgradients/right
   ...
   components except the 0-th have changed, while if wFi == Inf<Index>() then all
   the components, comprised the 0-th, have changed. */
...
/**< This method allows to change the entries corresponding to variables with
   ...
   the 0-th have changed, while if wFi == Inf<Index>() then all the components,
   comprised the 0-th, have changed.
