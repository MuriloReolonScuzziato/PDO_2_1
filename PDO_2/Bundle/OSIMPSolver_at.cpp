...
void OSIMPSolver::SetLowerBound( cHpNum LwBnd , cIndex wFi )
{
 ...
 if( LwBnd > -HpINF ) {
  if( wFi < InINF ) {  // insert the constraint gamma_i <= 1 and
   ...
   // be computed with respect to a <partial> stability center,
   // including the zero and difficult components but NOT the easy ones.
   // On the other hand, LwBnd passed as argument
   // is computed with respect to the whole stability center.
   // This means that we must add the easy contribution to LwBnd.
...
HpNum OSIMPSolver::ReadSigma( cIndex wFi )
{
 ...
 if( wFi > NrFi ) {  // aggregate the linearization error and the rhs of the
  ...
  // remark that is not necessary to divide any part of Sigma by rho
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
void OSIMPSolver::ChangeCurrPoint( cLMRow DLambda , cHpRow DFi )
{
 ...
 if( HasLwrBnd ) {  // recompute and set the lower bound
  ...
  if( EasyValue < HpINF )
   ...
   throw( NDOException( "OSIMPSolver::ChangeCurrPoint: this should not happen" ) );
