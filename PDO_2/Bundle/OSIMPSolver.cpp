...
/*--                                                                      --*/
/*--             Copyright (C) 2001 - 2015 by Antonio Frangioni           --*/
/*--                                                                      --*/
/*--------------------------------------------------------------------------*/
/*--------------------------- IMPLEMENTATION -------------------------------*/
/*--------------------------------------------------------------------------*/
...
/* Unfortunately, OsiSolverInterface objects do not do a bunch of things that
   ...
   solver can be used to do these. It's a bad hack (it always is when
...
OSIMPSolver::OSIMPSolver( istream *iStrm ) : MPSolver()
{
			  ...
			  const bool UsAvSt )
{
 ...
 if( Oracle ) {  // allocate the memory for solving the problem- - - - - - - -
  ...
  for( Index i = 0 ; i++ < NrFi ; ) {
   ...
   if( BNC ) {
    ...
    MSG( 0 , "Structured constraints for the easy component " << i
         << " have been created starting from the row "
	 << comp_row[ i - 1 ] << endl );
    }
   else {
    weasy[ i ] = false;
  ...
  for( Index i = 1 ; i <= NrFi ; i++ ) {
   BNC = FIO->GetBNC( i );
   if( BNC == 0 ) {
    // For each component "i" we must create a variable \f$gamma^i\f$. The
    // coefficient of the \f$gamma^i\f$ into objective is - lower bound of
    // the i-th component. The coefficient is 0 if no lower bound is given,
    // as in the rho case. However, unlike in the rho case, \f$gamma^i\f$
    // must to be set to 0 whenever some item exist.
...
inline void OSIMPSolver::SetPar( const int wp , cHpNum value )
{
 switch( wp ) {
  ...
  case( kFsbEps ):  // because the dual formulation is used, the role of
   ...
   osiSlvr->setDblParam( OsiDualTolerance , double( FsbEps ) );
    ...
    if( algorithm == kBar ) {
     ...
     CPXsetdblparam( env , CPX_PARAM_BAREPCOMP , value );
     }
   #endif
   break;
...
void OSIMPSolver::SetLowerBound( cHpNum LwBnd , cIndex wFi )
{
 ...
 if( LwrBnds[ h ] == LwBnd )  // re-setting some already known bound
  return;                     // nothing to do

 LwrBnds[ h ] = LwBnd;        // record the lower bound
 ...
 if( LwBnd > -HpINF ) {
  if( wFi < InINF ) {  // insert the constraint gamma_i <= 1 and
   ...
   // remark that the global lower bound entering objective function should
   // be computed with respect to a <partial> stability center, including
   // the zero and difficult components but NOT the easy ones. On the other
   // hand, LwBnd passed as argument is computed with respect to the whole
   // stability center. This means that we must add the easy contribution to
   // LwBnd.
   // Then, compute the easy part of the stability center- - - - - - - - - -
   //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   for( Index i = 0 ; i++ < NrFi ; )
    if( weasy[ i ] )
     if( FiLambda[ i - 1 ] < HpINF )
      EasyValue += FiLambda[ i - 1 ];
     else {
      EasyValue = HpINF;
      break;
      }

   // Set the global lower bound. If the easy part has not been computed yet
   // then totally ignore the lower bound. This should happen at the first
   // iteration when the value of the easy components are unknown. In fact,
   // the master problem computes the easy value and so at beginning the value
   // is unknown.
   ...
   if( EasyValue < HpINF ) {
    ...
    osiSlvr->setObjCoeff( comp_col[ 0 ] , - ( LwBnd + EasyValue ) );
    MSG( 0 , "Total LB = " << LwBnd + EasyValue << endl );
    }
   else {
    HasLwrBnd = false;
...
void OSIMPSolver::SetAlgo( const OsiAlg algo , const OsiRed reduc )
{
 ...
 if( stab != quadratic )  // choose CPLEX algorithm- - - - - - - - - - - - -
  ...
  OsiGrbSolverInterface *osiGrb =
                            dynamic_cast<OsiGrbSolverInterface*>( osiSlvr );
...
OSIMPSolver::MPStatus OSIMPSolver::SolveMP( void )
{
 ...
 if( MaxTime ) {
  // amazingly, there is no method in OsiSolverInterface to do this!
   ...
   OsiGrbSolverInterface *osiGrb =
                            dynamic_cast<OsiGrbSolverInterface*>( osiSlvr );
   GRBenv* env = osiGrb->getEnvironmentPtr();
   GRBsetdblparam( env, GRB_DBL_PAR_TIMELIMIT , MaxTime );
  #else
 ...
 if( ! osiSlvr->isProvenOptimal() ) {
   ...
   if( CPXgetstat( env , qp ) == CPX_STAT_ABORT_TIME_LIM ) {
    MSG( 0 , "MP stopped by time limit" << endl );
    return( kStppd );
    }
  #elif WHICH_OSISOLVER == 2
   OsiGrbSolverInterface *osiGrb =
...
void OSIMPSolver::RmvItem( cIndex i )
{
 ...
 if( ( ! ( NSubG[ wFi ] + NConst[ wFi ] ) ) &&
  ...
  MSG( 0 , "removed item " << i << " relative to the component " << wFi
           << endl );
  }
...
void OSIMPSolver::ChangeCurrPoint( cLMRow DLambda , cHpRow DFi )
{
 ...
 if( HasLwrBnd ) {  // recompute and set the lower bound
  // exclude the easy part - - - - - - - - - - - - - - - - - - - - - - - - -
  // (see the comments in SetLowerBound())

  HpNum EasyValue = 0;
  for( Index i = 0 ; i++ < NrFi ; )
  if( weasy[ i ] )
   if( ( FiLambda[ i - 1 ] < HpINF ) && ( FiLambda1[ i - 1 ] < HpINF ) )
    EasyValue += ( FiLambda1[ i - 1 ] - FiLambda[ i - 1 ] );
   else {
    EasyValue = HpINF;
    break;
    }

  // subtract the easy part from the stability center - - - - - - - - - - -

  if( EasyValue < HpINF ) {
   tempHP[ comp_col[ 0 ] ] += ( DFi[ 0 ] - EasyValue );
   *LwrBnds -= ( DFi[ 0 ] - EasyValue );
   }
  else
   throw( NDOException(
		 "OSIMPSolver::ChangeCurrPoint: this should not happen" ) );
  }
...
OSIMPSolver::~OSIMPSolver()
{
 ...
 #if( ( WHICH_OSISOLVER == 1 ) && OSIMPSOLVERLOG )
  CPXfclose( logfile );
 #endif
...
void OSIMPSolver::tUpdatePrices( cIndex strt , Index stp )
{
 ...
 if( stab == boxstep ) {
  ...
  if( stab == quadratic ) {
    ...
    GRBaddqpterms( model , CrrSGLen , tempI , tempI , tempHP );
   #else
