...
/*--                        Universita' di Pisa                           --*/
/*--                                                                      --*/
/*--             Copyright(C) 1992 - 2015 by Antonio Frangioni            --*/
/*--                                                                      --*/
/*--------------------------------------------------------------------------*/
/*--------------------------- IMPLEMENTATION -------------------------------*/
/*--------------------------------------------------------------------------*/
...
#define size( x ) stdsize() * fmax( ABS( x ) , HpNum( 1 ) )
#define gsize( x ) stdsize() * fmax( x , HpNum( 1 ) )
   ...
   on XXX, i.e. ABS( XXX ) < eXXX => XXX == 0. */
{
 ...
 if( M1j > eM1j )                  // M1j > 0 => t <= M2j / M1j
  tMax = fmin( tMax , M2j / M1j );
 else
  if( M1j < - eM1j )               // M1j < 0 => t >= M2j / M1j
   tMin = fmax( tMin , M2j / M1j );
...
MinQuad::MinQuad( void )
{
 ...
 MaxTime = 0;  // by default, no time limit
...
void MinQuad::SetMaxDim( Index m , Index n , Index SDim )
{
 ...
 {               // - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  CrrBDim = fmin( n , m );
  SgSpDim = ( SDim ? fmin( MaxBDim , Index( SDim + 3 ) ) : MaxBDim );
  ...
  for( Index j = 0 ; j < fmin( SgSpDim , CrrBDim ) ; j++ )
   #if( VASTE_MEM < 2 )
    L[ j ] = new HpNum[ j + 1 ]
   #else
...
void MinQuad::SetCrrBDim( cIndex Newn )
{
 ...
 if( Newn > CrrBDim ) {  // increasing active bundle dimension- - - - - - - -
  ...
  for( Index j = CrrBDim ; j < fmin( SgSpDim , Newn ) ; j++ )
   #if( VASTE_MEM < 2 )
    L[ j ] = new HpNum[ j + 1 ];
   #else
    L[ j ] = new HpNum[ SgSpDim ];
  ...
  for( Index h =  fmin( SgSpDim , CrrBDim ) ; h-- > Newn ; )
   delete[] L[ h ];
  }
...
MinQuad::MQError MinQuad::SolveQP( HpNum ti )
{
 ...
 for(;;) {
  ...
  if( f < HpINF )
   if( Lastf >= f * ( 1 + sgn( f ) * fmax( eR , 2 * HpEps ) ) ) {
  ...
  if( ( eR > MaxEpsR ) || ( eR < MinEpsR ) ) { 
   ...
   eR = fmin( MaxEpsR , fmax( MinEpsR , eR ) );
   QPStatus = kFatal;
   break;
   }
  ...
  if( f == HpINF ) {  // Base[] is changed: Mult[] may not be feasible
   ...
   if( BDim > CBDim ) {  // there are subgradients in Base[]
    ...
    if( LBIsInB )                           // the Lower Bound is in Base[]
     LBMult = fmax( 1 - ex , HpNum( 0 ) );   // it "absorbs" the change
 ...
 do {
  ...
  while( ! LocalOptima ) {
   /* The inner loop begins: the loop is repeated until a restricted optima
    ...
    Step++;
    SumBDim += BDim;
    SumGSBase += float( CBDim ) / float( fmax( BDim , Index( 1 ) ) );

    LOG3( endl << "(" << Calls << "|" << TmpStep++ << "|" << BDim << "|" \
          << Dependent << ")" );
   #endif

   // if too much time has passed in this call, return
   if( MaxTime && MQt )
    if( MQt->Read() > MaxTime ) {
     QPStatus = kStpTime;
     break;
   ...
   {                    //- - - - - - - - - - - - - - - - - - - - - - - - - -
                        // Mult = Mult + Step * v : the fmax feasible Step is
  ...
  if( ( Ename < MaxBDim ) && ( f < HpINF ) ) {
   ...
   if( Newf >= f * ( 1 - sgn( f ) * fmax( eR , 2 * HpEps ) ) ) {
  ...
  if( f * ( 1 + sgn( f ) * fmax( eR , 2 * HpEps ) ) <= Minf ) {
   #if( LOG_MQ > 4 )
    if( TLDim )
  ...
  Ename = InINF;
  HpNum minro = - fmax( eR * BDim * ABS( f ) , HpEps );
...
inline void MinQuad::MemDealloc( void )
{
 ...
 for( Index i = fmin( SgSpDim , CrrBDim ) ; i-- ; )
  delete[] L[ i ];
...
#define CMP( x1 , x2 ) ABS( x1 ) > eR * Lmu * fmax( ABS( x2 ) , HpNum( 1 ) )
...
#define CMPQ( x1 , x2 ) ABS( x1 - x2 ) > eR * fmax( ABS( x2 ) , HpNum( 1 ) )
