...
/*--                           Universita' di Pisa                        --*/
/*--                                                                      --*/
/*--             Copyright (C) 2001 - 2013 by Antonio Frangioni           --*/
/*--                                                                      --*/
/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
/*--------------------------- IMPLEMENTATION -------------------------------*/
...
static cIndex InINF = Inf<Index>();  // for the "empty" ActvVrs
             ...
             #endif
{
 ...
 SetPricing( ( CtOff < 0 ? Inf<HpNum>() : CtOff ) );
...
void QPPenaltyMP::SetDim( cIndex MxBSz, FiOracle *Oracle, const bool UsAvSt )
{
 ...
 if( MxBSz )
  ...
  {             //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   ...
   if( NrFi > 1 )
    throw( NDOException( "QPPenaltyMP::SetDim: decomposable Fi not supported"
			 ) );
   ...
   RHSxd = Inf<HpNum>();
    ...
    for( Index i = 0 ; i < CrrSGLen ; i++ ) {
     ...
     #else
      if( UB < Inf<LMNum>() )
       throw( NDOException( "QPPenaltyMP::SetDim: upper bounds not supported"
			    ) );
     #endif
     }
    ...
    for( Index i = 0 ; i < CrrSGLen ; i++ )
     if( ( ! Oracle->GetUC( i ) ) || ( Oracle->GetUB( i ) < Inf<LMNum>() ) )
      throw( NDOException(
...
void QPPenaltyMP::MakeLambda1( cHpRow Lmbd , HpRow Lmbd1 , cHpNum Tau )
{
 CompleteZ( false );  // ensure that everything is ready- - - - - - - - - - -
  ...
  if( ActvVrs && ( ActvVDim < CrrSGLen ) ) {
   LMRow tL = Lmbd;
   LMRow tL1 = Lmbd1;
   Index_Set tAB = ActvVrs;
   for( Index h ; ( h = *(tAB++) ) < Inf<Index>() ; )
    *(tL1++) = *(tL++) - Tau * dir[ h ];
   }
  else
...
HpNum QPPenaltyMP::CheckLinErr( cHpNum AEps , const bool All , cIndex wFi )
{
 HpNum mA = MinNewAlfa;
 MinNewAlfa = Inf<HpNum>();
...
SgRow QPPenaltyMP::GetItem( cIndex wFi )
{
 ...
 #endif
 G1Perz = Inf<HpNum>();   // signal that G1Perz has to be computed
 #if( G_IMPLM >= 3 )
  Insrtd = Inf<Index>();  // signal that from now on the content of tmpG1[]
...
{
 if( Nm == Inf<Index>() ) {  // special case: the constant subgradient- - - -
  if( MaxItemN() )           // - - - - - - - - - - - - - - - - - - - - - - -
   throw( NDOException(
		 "QPPenaltyMP::SetItem: RHS must be the very first item" ) );
...
void QPPenaltyMP::AddVars( cIndex NNwVrs )
{
 ...
 #if( G_IMPLM >= 3 )
  Insrtd = Inf<Index>();  // since tmpG1[] is used as a temporary, signal that it
                   // cannot be used any longer as G[ Insrtd ]
 #endif
 ...
 for( Index j = CrrSGLen ; j < NewSGLen ; j++ ) {
  ...
  #if( HV_NNVAR )  // - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   GetVars()[ j ] = ( ( ! UCV ) || ( UB < Inf<LMNum>() ) ? NNVar() : 0 );
   LowerBounds()[ j ] = - ( UCV ? Inf<LMNum>() : 0 );
   if( ! UCV )
    NNVars++;
...
void QPPenaltyMP::ChangeCurrPoint( cLMRow DLambda , cHpRow DFi )
{
 ...
 MinNewAlfa = Inf<HpNum>();  // reset the min negative Alfa, since they have changed
...
void QPPenaltyMP::ChgSubG( cIndex strt , Index stp , cIndex wFi )
{
 ...
 if( ! NrFi )
  throw( NDOException( "ChgSubG: changing RHS only is not supported" ) );
...
{
 if( RHSxd == Inf<HpNum>() )  // still have to compute RHS * d
  if( RHSp )
   #if( HV_NNVAR )
    RHSxd = DPerG( RHSp );
   #else
