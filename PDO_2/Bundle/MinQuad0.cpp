...
static inline void VectAdd( HpRow g , cHpRow g1 ,
			    cHpRow g2 , cHpNum x ,
			    cHpNum y , Index n )
/* g := x * g1 + y * g2 */
{
...
void MinQuad::ResetBundle( cIndex which )
{
 ...
 if( G[ which ] & IsInBse ) {
  ...
  Minf = f = Inf<HpNum>();     // Mult[] must be changed
  }
...
void MinQuad::ChangeAlfa( cHpNum DeltaAlfa )
{
 ...
 Lin = Inf<HpNum>();  // Alfa is changed (but z2 is correct)
...
void MinQuad::AddSGSpaceDim( cSgRow NewDim , const bool UpdtB )
{
 ...
 Index i;
 for( Index_Set tO = Order ; ( i = *(tO++) ) < Inf<Index>() ; ) {
  SgNum NDi = NewDim[ i ];
 ...
 Quad = Inf<HpNum>();  // f() must be changed: the "new" Quad is just
...
void MinQuad::AddSGSpaceDim( cSgRow NewDim , cHpNum lbh , const bool UpdtB )
{
 ...
 Index i;
 for( cIndex_Set tO = Order ; ( i = *(tO++) ) < Inf<Index>() ; ) {
  SgNum NDi = NewDim[ i ];
 ...
 {
  Index h;
  for( cIndex_Set tO = Order ; ( h = *(tO++) ) < Inf<Index>() ; )
   Alfa[ h ] += NewDim[ h ] * lbh;
  }
 ...
 Quad = Inf<HpNum>();  // f() must be changed
...
void MinQuad::ChangeCoefficient( cIndex i , cHpNum ci )
{
 if( ( i < CrrBDim ) && ( cf[ i ] != ci ) && G[ i ] )
  if( ci >= 0 ) {
   ...
   if( G[ i ] & IsInBse )
    f = z1Norm = Inf<HpNum>();  // Mult and z1 must be changed
   }
...
MinQuad::MQError MinQuad::SolveQP( HpNum ti )
{
 ...
 for(;;) {
  ...
  if( f < Inf<HpNum>() )
   if( Lastf >= f * ( 1 + sgn( f ) * max( eR , 2 * Eps<HpNum>() ) ) ) {
  ...
  Minf = Inf<HpNum>();  // ... and the old value of Minf shall not be trusted
   ...
   disregarded, but the corresponding items are discarded from Base[]. */
{
 ...
 for( bp = Base ; ( i = *(bp++) ) < Inf<Index>() ; ) {
  ...
  if( ( Delta > 0 ) ||
      ( ( ! Delta ) && ( ( Dependent < MxDep ) || ( *bp == Inf<Index>() ) ) ) ) {
 ...
 if( SGDltd )
  f = Inf<HpNum>();     // Mult[] are unfeasible
 else
  Quad = Inf<HpNum>();  // just to be more exact, recompute Quad
   ...
   big, so the caller should decrease it. */
{
 ...
 {                      //- - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  if( f == Inf<HpNum>() ) {  // Base[] is changed: Mult[] may not be feasible
   LocalOptima = false;
   LOG3( " ~ diff. B" );
   ...
   #if( VARCOEFF )
    if( z1Norm == Inf<HpNum>() ) {  // the coefficients have changed
     Minf = Inf<HpNum>();
  ...
  if( Quad == Inf<HpNum>() ) {  // Q[] is changed- - - - - - - - - - - - - - -
   Minf = f = Inf<HpNum>();     // signal not to check for decrease of f(),
   LocalOptima = false;  // but avoid to recalculate Quad now (a costly task)
   LOG3( " ~ diff. Q" );
   }

  if( Lin == Inf<HpNum>() ) {  // Alfa is changed- - - - - - - - - - - - - - -
   Minf = Inf<HpNum>();
   LocalOptima = false;

   if( z1Tz2 == Inf<HpNum>() ) {  // and also z2 is changed
 ...
 Ename = Inf<Index>();     // reset the entering item
 ...
 do {
  ...
  while( ! LocalOptima ) {
   /* The inner loop begins: the loop is repeated until a restricted optima
      ...
      If Ename < Inf<Index>() the solution may have the form [ x 0 ], if the
      ...
      (=> LastRo < - Inf<HpNum>()) then it will contain at least a
   ...
   if( LocalOptima )       // tmpv contains a point- - - - - - - - - - - - -
    ...
    if( ( Ename < Inf<Index>() ) && ( Mult[ Ewher ] == 0 ) ) {  // Mult[] has the
  ...
  if( ( Ename < MaxBDim ) && ( f < Inf<HpNum>() ) ) {
   /* Test for *strict*  decrease of f(), i.e. Newf < f - eR * | f |: skip the
      ...
      The test is also skipped when f == Inf<HpNum>(): this can happen during a
      "phase transition", i.e. when the first subgradient enters in Base. */

   if( Newf >= f * ( 1 - sgn( f ) * max( eR , 2 * Eps<HpNum>() ) ) ) {
  ...
  if( f * ( 1 + sgn( f ) * max( eR , 2 * Eps<HpNum>() ) ) <= Minf ) {
   #if( LOG_MQ > 4 )
    if( TLDim )
  ...
  if( LastRo > - Inf<HpNum>() )  // if Mult[] is feasible
  ...
  Ename = Inf<Index>();
  HpNum minro = - max( eR * BDim * ABS( f ) , HpNum( Eps<HpNum>() ) );
  cHpNum brkro = - ( EpsRo == Inf<HpNum>() ? EpsRo : EpsRo * BDim * ABS( f ) );
   ...
   if( ! ( G[ h = *(--tO) ] & IsInBse ) ) {  // a nonbasic item - - - - - - -
    ...
    if( ! ( G[ h ] & IsACnst ) )
     if( LastRo > - Inf<HpNum>() )
      ...
      ro = - Inf<HpNum>();      // the first subgradient entering in Base
      Minf = f = Inf<HpNum>();  // switch to the "true" f() (hence reset Minf)
      }
  ...
  if( ( Ename < Inf<Index>() ) && ( LwrBnd < Inf<HpNum>() ) && ( ! LBIsInB ) )
  {
   HpNum ro = LwrBnd / ti;
  ...
  if( Ename < Inf<Index>() ) {  // a dual constraint is violated- - - - - - -
   LocalOptima = false;  // - - - - - - - - - - - - - - - - - - - - - - - - -
