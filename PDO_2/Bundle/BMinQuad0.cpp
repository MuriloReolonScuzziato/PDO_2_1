...
/*--   			     Universita' di Pisa			  --*/
/*--                                                                      --*/
/*--               Copyright 1992 - 2012 by Antonio Frangioni             --*/
/*--                                                                      --*/
/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
/*--------------------------- IMPLEMENTATION -------------------------------*/
...
void BMinQuad::ChangeBounds( void )
{
 ...
 for( Index h ; ( h = *(tB2++) ) < Inf<Index>() ; ) {
   ...
   if( ABS( bounds[ h ] ) == Inf<LMNum>() )  // meanwhile, check if some variable
    MvdVars[ Moved++ ] = h;           // has in fact become unconstrained
   else
  #endif
...
void BMinQuad::AddVars( cIndex_Set whch , cIndex hwmny )
{
 ...
 {              //- - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  const bool UptdB = ( tMB2Dim < 2 * ReadBDim() / 3 );  // true if updating the
                                                   // base is convenient
  for( ; ( i = *(MVD++) ) < Inf<Index>() ; )
...
void BMinQuad::RemoveVars( cIndex_Set whch , Index hwmny )
{
  ...
  *tMV2 = Inf<Index>();
  ...
  for( tw = whch ; ( i = *(tw++) ) < Inf<Index>() ; )  // declare them all out
   GS[ i ] &= kIsNN;                            // keep the first bit
...
{
 if( *whch == Inf<Index>() )  // no variables to remove
 ...
 assert( *tB2 == Inf<Index>() );
 assert( *tMB2 == Inf<Index>() );
...
void BMinQuad::AddD( LMRow L1 , cLMRow L2 , cHpNum Tau )
{
 ...
 if( Tau == PrvsTi )
  ...
  for( HpNum TauR = Tau / PrvsTi ; ( h = *(o++) ) < Inf<Index>() ; )
...
inline void BMinQuad::CalcOptDir( HpNum ti )
{
 ...
 HpNum BfBot = Inf<HpNum>();  // "security level", used to switch between the "quick
 ...
 Index Moved = 0;      // number of variables moved from Base2 to MBase2 or
                       // vice-versa
 Index Entrd = Inf<Index>();  // name of a variable entered in Base2 in the latest
                       // iteration
 do {
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  /*- - - - - - - - - - - - - - Inner Loop  - - - - - - - - - - - - - - - -*/
  ...
  do {
     ...
     *BMQLog << endl << "[" << BCalls << "/" << BStep++ << "-" << B2Dim
     ...
     if( Bf < Inf<HpNum>() ) *BMQLog << Bf << endl;
   ...
   if( BfBot < Inf<HpNum>() )  // "secure" pricing - - - - - - - - - - - - - - - - -
   {                    //- - - - - - - - - - - - - - - - - - - - - - - - - -
    #if( LAZY_D == 0 )
     CalculateZ( tmpdi );
    #elif( LAZY_D == 1 )
  ...
  if( Bf < Inf<HpNum>() ) {  // skipping the tests if Bf is undefined
   ...
   cHpNum RqrdDcrs = max( MnDcrs ,
			  Eps<HpNum>() * max( BDim , Index( 1 ) ) * ABS( Bf ) );
   ...
   if( Bf - newBf < - RqrdDcrs ) {  // an increasing step
    ...
    if( BfBot == Inf<HpNum>() ) {  // it's the first increasing step
    ...
    if( Bf - newBf <= RqrdDcrs ) {   // a "short" step
     ...
     if( newBf + eD * ( ABS( newBf ) / SpaceDim ) < BfBot ) {
      ...
      #endif
      BfBot = Inf<HpNum>();       // it is safe to disable the "secure" rule ...
...
inline void BMinQuad::RmvFrmB2( cIndex_Set MVD , cIndex MVDd )
{
 ...
 for( ; h < Inf<Index>() ; tB2r++ )
  if( *tB2r == h )
   h = *(++MVD);
  else
   *(tB2w++) = *tB2r;

 while( ( h = *(tB2r++) ) < Inf<Index>() )
  *(tB2w++) = h;
...
inline void BMinQuad::CutOffConstrs( cIndex_Set MVD , Index MVDd )
{
 ...
 const bool UptdB = ( MVDd < 2 * ReadBDim() / 3 );  // true if updating the base
                                               // is convenient
 for( Index h ; ( h = *(MVD++) ) < Inf<Index>() ; )
...
{
 #if( CHECK_DS & 1 )
  assert( MBase2[ MB2Dim ] == Inf<Index>() );
