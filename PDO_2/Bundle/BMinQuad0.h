...
/* At each iteration, variables are added to / removed from the "active set",
   ...
   If the methods are not called, "Inf<Index>()" is assumed for both limits, meaning
   "no limit at all". */
...
/* These methods add `hwmny' new variables to the set of declared variables.
   ...
   Inf<Index>()-terminated (that is, whch[ hwmny ] == Inf<Index>()). The second adds all
   those with names from `strt' to `strt + hwmny - 1', extremes included, and
   it can only be called if strt is *larger* than the name of any declared
   variable. Thus, for the second form none of the variable can be declared
   just prior to the call of the method; the same has to be true for the
   first form.
...
/* Renames all the variables to make the set of variable names fit with the
   ...
   (ordered in increasing sense, without duplications and Inf<Index>()-terminated).
...
/* Dealing with constraints on the direction d requires at least checking if
   ...
   is guaranteed to be Inf<Index>()-terminated, i.e., Base[ BDim ] == Inf<Index>().
