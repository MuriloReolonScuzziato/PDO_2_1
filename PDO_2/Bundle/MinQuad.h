 ...
 *         Universita' di Pisa \n
 *
 * Copyright &copy 1992 - 2015 by Antonio Frangioni.
 */
/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
/*----------------------------- DEFINITIONS --------------------------------*/
...
/** Public enum describing the possible return values of SolveQP() [see
    below]. */
...
/**< This method must be called prior to any other method in the class (but
   the constructor) to tell the MinQuad object the size of the problem to be
...
/**< Sets the max. *active* dimension of the bundle, that must always be <=
   ...
   >= Newn in the bundle, they are simply deleted.
...
/**< Sets a lower bound on the value of the variable v in the QP (the
   predicted value of the Cutting Plane model). This lower bound is
   automatically updated when the current point is changed [see MoveAlongD()
   below]. Passing - Inf<HpNum>()() as the lower bound means that no such
   bound is available.
...
/*--------------------------------------------------------------------------*/
   ...
   inline void SetMaxTime( cHpNum MaxTime );

/**< Sets the maximum time (in seconds) that each call to SolveQP() [see
   below] is allowed to last. If this method is never called, 0 is assumed,
   which means "no time limit". Note that this only works if time is
   actually recorded, see SetMQTime() below. */
...
/**< Changes the Alfa (linearization error of a subgradient or RHS of a
   constraint) of the item with name 'i', adding it DeltaAlfai; that is,
...
/**< Performs the same task of AddSGSpaceDim( cSgRow , const bool ) [see
   above] for the case where MinQuad is used as the "core solver" for the
   box-constrained case. It handles the case when a box-constrained active
   variable have to be *deleted* from the Base, where lbh is its lower bound
   (or - its upper bound). */
...
/**< Performs the same task of CutSGSpaceDim( cSgRow , const bool ) [see
   above] for the case where MinQuad is used as the "core solver" for the
   box-constrained case. It handles the case when a box-constrained active
   variable have to be *deleted* from the Base, where lbh is its lower bound
   (or - its upper bound). */
...
/**< The current matrix Q is substituted with an entirely new Q, whose
   elements are requested with calls to GiTG[j]() [see below]. This can be a
   cheaper alternative to the use of [Add/Cut]SGSpaceDim() [see above] in the
   case of many changes to the Q matrix, or be simply used to insert an
   entirely uncorrelated problem.
...
/* After that data of the problem has been changed with any sequence of calls
   to [Add/Cut]GSpaceDim( ... , false ) or ChangeQ() [see above], either this
   method or ResetB() [see below]  must be called to ensure that the data
   structures describing the current base are updated.
...
/* After that data of the problem has been changed with any sequence of calls
   to [Add/Cut]GSpaceDim( ... , false ) or ChangeQ() [see above], either this
   method or UpdateB() [see above]  must be called to ensure that the data
   structures describing the current base are updated.
...
/**< Solves the problem with the current data, i.e. the current bundle, Alfa
   ...
   - kQPPrimUnbndd if the problem is proven to be primal unbounded. This can
     ...
     the bundle. In this case, the method ReadInfDir() [see below] can be
     used together with the solution returned by ReadMult() [see below] to
     construct an unbounded ray of the problem.

   - kStpTime if the maximum time limit set with SetMaxTime() [see above]
     has elapsed before the algorithm can find an optimal solution (or
     prove the problem to be unbounded). This is not a "serious" condition,
     in that it can be "cured" by just calling again (possibly repeatedly)
     the method to give it more time (since warm starts are done, this
     has a limited overhead w.r.t. directly calling the method once with
     a longer time limit).

   - kFatal if even adjusting eR was not sufficient to handle the
     numerical problems, i.e., either eR is increased/decreased out of the
     bounds or two contrasting requests (increase eR and then decrease it
     or vice-versa) have been detected. */
...
/**< After (correct) termination of SolveQP(), ReadzNorm() returns
   || z^* ||^2. Together with ReadSigma() [see below] this allows to obtain
...
/* After (correct) termination of SolveQP(), ReadSigma() returns Sigma^*.
   Together with ReadzNorm() [see above] this allows to obtain the optimal
...
/**< After (correct) termination of SolveQP(), returns a pointer to a
   ...
   are returned respectively by ReadBDim() and ReadBase() [see below].
...
/**<  Returns the number of non-zero entries of x^*, i.e., the length of the
   vector returned by ReadMult() [see above]. */

/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */

   inline cIndex_Set ReadBase( void );

/**< Returns the optimal base. That is, ReadMult()[ i ] contains the
   multiplier relative to theitem whose "name" is ReadBase()[ i ] for
...
/*--------------------------------------------------------------------------*/
  ...
  inline void GjTGBk( cIndex j , cIndex_Set Bk , Index Dim , HpRow v );
...
inline void MinQuad::SetMaxTime( cHpNum NewMaxTime )
{
