...
Spcdec2Results::Spcdec2Results(CSistema * const sistema_end)
{
	...
	na = N * (sistema_a->termeletricasVtr.size() + (1+flag3)*sistema_a->hidreletricasVtr.size());
	nd = N * (sistema_a->termeletricasVtr.size() + (1+flag3)*sistema_a->hidreletricasVtr.size() + (1 - flag3));		// numero de variáveis duais (e n de variaveis auxiliares/duplicadas)
	...
	xa.resize(na);
	obj = 0;
	obj_subp.resize(CH + I + N);
	for (int i = 0; i <  CH + I + N; i++)
		obj_subp[i] = 0;
	ptr_x_subp.resize(CH + I + N);
	ptr_x_subp_agg.resize(1);
	ptr_x_subp_agg[0].resize(n + na);
	
	//AjustarPreconditioners();

	//ptr_x_hat = new vetorfloat[CH + I + N];		// iniciar com esse tamanho e depois fazer um resize em SetComponents()
	//ptr_x_til = new vetorfloat[CH + I + N];
...
Spcdec2Results::~Spcdec2Results(void)
{
	delete inFile;
}
...
// subproblema H comp = id1;					// id1 é o índice da cascata de hidros //x = [ph v d s (phmax) phg q z vfol]
// subproblema T comp = CH + id1;				// id1 é o índice da termeletrica	//x = [pt u up ud F]
// subproblema D comp = CH + I + id1;			// id1 é o índice do nó	//x = [pt teta ph (phmax) def]

void Spcdec2Results::SetComponents( int * comp_inf, bool aggr )
{
	Aggrgtd = aggr;
...
void Spcdec2Results::GravarSolucao(double obj_a, vetorfloat x_a, int status_a, int comp)
{
	// Grava o resultado de cada subproblema
	// subproblema H comp = id1;					// id1 é o índice da cascata de hidros
	// subproblema T comp = CH + id1;				// id1 é o índice da termeletrica
	// subproblema D comp = CH + I + id1;			// id1 é o índice do nó
	status = status_a;
...
void Spcdec2Results::GetSubGradiente(int wFi_a, double * SubG)
{
	// verificar o calculo dos subgradientes abaixo!!
	...
	// 1 <= wFi (comp) <= GetNrFi();
	// lambda [pt ph (phmax || res)] -> tamanho: [I R (R || 1)] x T
	int delta = 0;
	int deltaa = 0;
	...
	if (flag1 == 1)
		flag1a = 1;
	if ( (wFi_a > 0) && (wFi_a <= CH) )	//Hidreletrico ( 1 <= comp <= CH )	//x = [ph v d s (phmax) phg q z vfol]
	{
		int ch = wFi_a - 1;
		int jj;
		double soma;
		nt = (ptr_x_subp[wFi_a - 1].size() - flag2*sistema_a->GetNCenarios()*cascata[ch].size()) / N;
		deltaa = sistema_a->termeletricasVtr.size();
		int JJJ = 0;
		for (int r = 0; r < cascata[ch].size(); r++)
			JJJ += sistema_a->hidreletricasVtr[cascata[ch].at(r)].GetNGrupos();		// numero total de grupos da cascata
		for (int t = 0; t < N; t++)
		{
			jj = delta;
			soma = 0;
			...
			{
				SubG[cascata[ch].at(r) + deltaa] = ptr_x_subp[ch][r + delta];						//ph
				if (sistema_a->GetFlagPhmax() == 1)
					SubG[cascata[ch].at(r) + sistema_a->hidreletricasVtr.size() + deltaa] = ptr_x_subp[ch][r + 4*cascata[ch].size() + delta];		//phmax
				else
				{
					for (int j = 0; j < sistema_a->hidreletricasVtr[cascata[ch].at(r)].GetNGrupos(); j++)
						soma += ptr_x_subp[ch][4*cascata[ch].size() + 2*JJJ + jj + j]*sistema_a->hidreletricasVtr[cascata[ch].at(r)].grupoVtr[j].GetPmax()*sistema_a->hidreletricasVtr[cascata[ch].at(r)].grupoVtr[j].GetNUnidades();		// z
					jj += sistema_a->hidreletricasVtr[cascata[ch].at(r)].GetNGrupos();
					soma += - ptr_x_subp[ch][r + delta];
				}
			}
			if (sistema_a->GetFlagPhmax() == 0)
				SubG[sistema_a->hidreletricasVtr.size() + deltaa] = soma;		//reserva
	...
	else if ( (wFi_a > CH) && (wFi_a <= CH + I) )		//Termeletrico	//x = [pt u cp F] ou [pt u up ud F]
	{
		int i = wFi_a - CH - 1;
		deltaa = 0;
		delta = 0;
	...
	else if ( (wFi_a > CH + I) && (wFi_a <= CH + I + N) )		//Demanda	//x = [pt teta ph (phmax) def]
	{
		int no = wFi_a - CH - I - 1;
		int cen = 0;
		...
		for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
		{
			...
			if (sistema_a->GetFlagPhmax() == 1)
				SubG[r + sistema_a->hidreletricasVtr.size() + deltaa] = - ptr_x_subp[wFi_a - 1][r + sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1) + sistema_a->hidreletricasVtr.size()];	//phmax
		}
	}
...
void Spcdec2Results::GetSubGradiente(double * SubG)
{
	int deltaa = 0;
	int delta = 0;
	int cen = 0;
	...
	for (int t = 0; t < N; t++)
	{
		if (sistema_a->GetFlagTbinaryModel() == 0)
			for (size_t i = 0; i < sistema_a->termeletricasVtr.size(); i++)
				SubG[i + deltaa] = - ptr_x_subp[t + CH + I][i] + ptr_x_subp[i + CH][t*(3 + flag4)];		// - pta + pt
		else
			for (size_t i = 0; i < sistema_a->termeletricasVtr.size(); i++)
				SubG[i + deltaa] = - ptr_x_subp[t + CH + I][i] + ptr_x_subp[i + CH][t*(3 + flag4 + flag7)] + ptr_x_subp[i + CH][t*(3 + flag4 + flag7) + 1]*sistema_a->termeletricasVtr[i].GetPmin();		//- pta + pt + u*pt_min
		deltaa += sistema_a->termeletricasVtr.size();
		double som = 0;
		for (size_t ch = 0; ch < cascata.size(); ch++)
		{
			...
			if (sistema_a->GetFlagPhmax() == 0)
			{
				jj = 4*cascata[ch].size() + delta;
				for (size_t r = 0; r < cascata[ch].size(); r++)
					jj += 2*sistema_a->hidreletricasVtr[cascata[ch].at(r)].GetNGrupos();
			}
			for (size_t r = 0; r < cascata[ch].size(); r++)
			{
				SubG[cascata[ch].at(r) + deltaa] = - ptr_x_subp[t + CH + I][cascata[ch].at(r) + sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1)] + ptr_x_subp[ch][r + delta];		// - pha + ph
				if (sistema_a->GetFlagPhmax() == 1)
					SubG[cascata[ch].at(r) + sistema_a->hidreletricasVtr.size() + deltaa] = - ptr_x_subp[t + CH + I][cascata[ch].at(r) + sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1) + sistema_a->hidreletricasVtr.size()] + ptr_x_subp[ch][r + 4*cascata[ch].size() + delta];		// - phmaxa + phmax
				else
				{
					for (int j = 0; j < sistema_a->hidreletricasVtr[cascata[ch].at(r)].GetNGrupos(); j++)
						som += ptr_x_subp[ch][jj + j]*sistema_a->hidreletricasVtr[cascata[ch].at(r)].grupoVtr[j].GetPmax()*sistema_a->hidreletricasVtr[cascata[ch].at(r)].grupoVtr[j].GetNUnidades();		// z
					jj += sistema_a->hidreletricasVtr[cascata[ch].at(r)].GetNGrupos();
					som += - ptr_x_subp[ch][r + delta];
				}
			}
		}
		if (sistema_a->GetFlagPhmax() == 0)
			SubG[sistema_a->hidreletricasVtr.size() + deltaa] = som;		//reserva: sum_r(ph_r - sum_j(phg_max*z_j))
		deltaa -= sistema_a->termeletricasVtr.size();
		deltaa += nat;
	}
...
double Spcdec2Results::GetFobj()
{
	obj = 0;
	for (int sp = 0; sp < CH + I + N; sp++)
		obj += obj_subp[sp];
	return obj;
}
vetorfloat Spcdec2Results::GetCompX(int comp)
{
	if ( Aggrgtd )
	{
		AlocarPtrXHat();
...
vetorfloat Spcdec2Results::GetX_med()
{
...
vetorfloat Spcdec2Results::GetX_med(bool til)
{
	...
	if ( Aggrgtd )
	{
		...
		for (int t = 0; t < N; t++)
		{
			...
			for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
			{
				...
				if (sistema_a->GetFlagPhmax() == 1)
					x_med[r + (4+flag3)*sistema_a->hidreletricasVtr.size() + delta] = (ptr_x_a[0][r + (4+flag3)*sistema_a->hidreletricasVtr.size() + delta] + ptr_x_a[0][n + r + sistema_a->hidreletricasVtr.size() + deltaa]) / 2;		//phmax
			}
			delta -= ((3+flag4+flag7)*sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1));
			deltaa -= sistema_a->termeletricasVtr.size();
			if (((t + 1 - sistema_a->GetTt1())%(sistema_a->GetTt2() - sistema_a->GetTt1()) == 0) && ((t + 1 - sistema_a->GetTt1()) > 0))
	...
	else
	{
		...
		for (int t = 0; t < N; t++)
		{
			if (sistema_a->GetFlagTbinaryModel() == 0)
			{
				for (int i = 0; i < I; i++)
				{
					x_med[i + delta] = (ptr_x_a[t + CH + I][i] + ptr_x_a[i + CH][t*(3 + flag4)]) / 2;				//pt
					x_med[i + I + delta] = ptr_x_a[i + CH][t*(3 + flag4) + 1];										//u		
					x_med[i + 2*I + delta] = ptr_x_a[i + CH][t*(3 + flag4) + 2];									//cp
				}
				if (sistema_a->GetFlagInitAproxCT() > 1)
					for (int i = 0; i < I; i++)
						x_med[i + 3*I + delta] = ptr_x_a[i + CH][t*(3 + flag4) + 3];								//F
			}
			else
			{
				for (int i = 0; i < I; i++)
				{
					x_med[i + delta] = (max(ptr_x_a[t + CH + I][i] - sistema_a->termeletricasVtr[i].GetPmin(), 0.0) + ptr_x_a[i + CH][t*(3 + flag4 + flag7)]) / 2;		//pt
					x_med[i + I + delta] = ptr_x_a[i + CH][t*(3 + flag4 + flag7) + 1];										//u		
					x_med[i + 2*I + delta] = ptr_x_a[i + CH][t*(3 + flag4 + flag7) + 2];									//up
					x_med[i + 3*I + delta] = ptr_x_a[i + CH][t*(3 + flag4 + flag7) + 3];									//ud
				}
				if (sistema_a->GetFlagInitAproxCT() > 1)
					for (int i = 0; i < I; i++)
						x_med[i + (3 + flag4 + flag7)*I + delta] = ptr_x_a[i + CH][t*(3 + flag4 + flag7) + 3 + flag7];								//F
			}
			if (sistema_a->GetFlagModeloRede() > 0)
			{
				for (size_t b = 0; b < sistema_a->barrasVtr.size(); b++)
					x_med[b + I*(3 + flag4 + flag7) + flag1a*(sistema_a->barrasVtr.size() - 1) + (4+flag3)*R + 3*JJ + delta] = ptr_x_a[t + CH + I][b + I + flag1a*(sistema_a->barrasVtr.size() - 1) + (1+flag3)*R];	//def
			}
			else
				x_med[I*(3 + flag4 + flag7) + flag1a*(sistema_a->barrasVtr.size() - 1) + (4+flag3)*R + 3*JJ + delta] = ptr_x_a[t + CH + I][I + (1+flag3)*R];	//def
			if (sistema_a->GetFlagModeloRede() == 1)
			{
				for (size_t b = 0; b < sistema_a->barrasVtr.size() - 1; b++)
					x_med[b + I*(3 + flag4 + flag7) + delta] = ptr_x_a[t + CH + I][b + I];	//teta
			}
			for (size_t ch = 0; ch < cascata.size(); ch++)
			{
				...
				delta += I*(3 + flag4 + flag7) + flag1a*(sistema_a->barrasVtr.size() - 1);
				for (size_t r = 0; r < cascata[ch].size(); r++)
				{
					x_med[cascata[ch].at(r) + delta] = (ptr_x_a[ch][r + deltaa] + ptr_x_a[t + CH + I][cascata[ch].at(r) + I + flag1a*(sistema_a->barrasVtr.size() - 1)]) / 2;	//ph
					x_med[cascata[ch].at(r) + R + delta] = ptr_x_a[ch][r + cascata[ch].size() + deltaa];											//v
					x_med[cascata[ch].at(r) + 2*R + delta] = ptr_x_a[ch][r + 2*cascata[ch].size() + deltaa];										//d
					x_med[cascata[ch].at(r) + 3*R + delta] = ptr_x_a[ch][r + 3*cascata[ch].size() + deltaa];										//s
					if (sistema_a->GetFlagPhmax() == 1)
						x_med[r + 4*R + delta] = (ptr_x_a[t + CH + I][cascata[ch].at(r) + I + flag1a*(sistema_a->barrasVtr.size() - 1) + R] + ptr_x_a[ch][r + 4*cascata[ch].size() + deltaa]) / 2;	//phmax

					jj = 0;
					for (int rr = 0; rr < cascata[ch].at(r); rr++)
						jj += sistema_a->hidreletricasVtr[rr].GetNGrupos();		// soma de tds os grupos que estão antes da usina cascata[ch].at(r)
					
					for (int j = 0; j < sistema_a->hidreletricasVtr[cascata[ch].at(r)].GetNGrupos(); j++)
					{
						x_med[(4+flag3)*R + j + jj + delta] = ptr_x_a[ch][(4+flag3)*cascata[ch].size() + j + jjj + deltaa];							//phg
						x_med[(4+flag3)*R + j + JJ + jj + delta] = ptr_x_a[ch][(4+flag3)*cascata[ch].size() + j + JJJ + jjj + deltaa];				//q
						x_med[(4+flag3)*R + j + 2*JJ + jj + delta] = ptr_x_a[ch][(4+flag3)*cascata[ch].size() + j + 2*JJJ + jjj + deltaa];			//z
					}
					jjj += sistema_a->hidreletricasVtr[cascata[ch].at(r)].GetNGrupos();
				}
				delta -= (I*(3 + flag4 + flag7) + flag1a*(sistema_a->barrasVtr.size() - 1));
			}
			if (((t + 1 - sistema_a->GetTt1())%(sistema_a->GetTt2() - sistema_a->GetTt1()) == 0) && ((t + 1 - sistema_a->GetTt1()) > 0))
		...
		if (sistema_a->GetFlagVfol() == true)
		{
			for (size_t ch = 0; ch < cascata.size(); ch++)
			{
				...
				{
					for (size_t r = 0; r < cascata[ch].size(); r++)
						x_med[cascata[ch].at(r) + I*(3 + flag4 + flag7) + flag1a*(sistema_a->barrasVtr.size() - 1) + (4+flag3)*R + 3*JJ + flag1d*sistema_a->barrasVtr.size() + (1 - flag1d) + delta] = ptr_x_a[ch][r + (4+flag3)*cascata[ch].size() + 3*JJJ + deltaa];		//vfol
...
void Spcdec2Results::AlocarX_med()
{
	// Alocar x_med a partir de x e xa
	AlocarXeXa();
	// Para as variáveis duplicadas x_med = (x + xa) / 2
	...
	for (int t = 0; t < N; t++)
	{
		...
		for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
		{
			...
			if (sistema_a->GetFlagPhmax() == 1)
				x_med[r + sistema_a->hidreletricasVtr.size() + delta] = (x[r + (4+flag3)*sistema_a->hidreletricasVtr.size() + delta] + xa[r + sistema_a->hidreletricasVtr.size() + deltaa]) / 2;		//phmax
		}
		delta -= (3+flag4)*sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1);
		deltaa -= sistema_a->termeletricasVtr.size();
		if (((t + 1 - sistema_a->GetTt1())%(sistema_a->GetTt2() - sistema_a->GetTt1()) == 0) && ((t + 1 - sistema_a->GetTt1()) > 0))
...
void Spcdec2Results::AlocarPtrXHat()
{
	...
	if (flag1 == 1)
		flag1a = 1;
	for (int comp = 0; comp < CH; comp++)	//Hidreletrico
	{
		// comp é o índice da cascata
		int id1 = comp;
		delta = (3 + flag4)*sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1);
		delta_subP = 0;
		nt_subP = (ptr_x_subp[id1].size() - flag2*sistema_a->GetNCenarios()*cascata[id1].size()) / N;
		int jjj, JJJ, jj;
		JJJ = 0;
		for (int r = 0; r < cascata[id1].size(); r++)
			JJJ += sistema_a->hidreletricasVtr[cascata[id1].at(r)].GetNGrupos();		// numero total de grupos da cascata
		...
		for (int t = 0; t < N; t++)
		{
			...
			{
				ptr_x_subp_agg[0][cascata[id1].at(r) + delta] = ptr_x_subp[id1][r + delta_subP];												//ph
				ptr_x_subp_agg[0][cascata[id1].at(r) + sistema_a->hidreletricasVtr.size() + delta] = ptr_x_subp[id1][r + cascata[id1].size() + delta_subP];			//v
				ptr_x_subp_agg[0][cascata[id1].at(r) + 2*sistema_a->hidreletricasVtr.size() + delta] = ptr_x_subp[id1][r + 2*cascata[id1].size() + delta_subP];		//d
				ptr_x_subp_agg[0][cascata[id1].at(r) + 3*sistema_a->hidreletricasVtr.size() + delta] = ptr_x_subp[id1][r + 3*cascata[id1].size() + delta_subP];		//s
				if (sistema_a->GetFlagPhmax() == 1)
					ptr_x_subp_agg[0][cascata[id1].at(r) + (4+flag3)*sistema_a->hidreletricasVtr.size() + delta] = ptr_x_subp[id1][r + 4*cascata[id1].size() + delta_subP];		//phmax

				jj = 0;
				for (int rr = 0; rr < cascata[id1].at(r); rr++)
					jj += sistema_a->hidreletricasVtr[rr].GetNGrupos();		// soma de tds os grupos que estão antes da usina cascata[ch].at(r)

				for (int j = 0; j < sistema_a->hidreletricasVtr[cascata[id1].at(r)].GetNGrupos(); j++)
				{
					ptr_x_subp_agg[0][cascata[id1].at(r) + (4+flag3)*sistema_a->hidreletricasVtr.size() + jj + j + delta] = ptr_x_subp[id1][(4+flag3)*cascata[id1].size() + j + jjj + delta_subP];					//phg
					ptr_x_subp_agg[0][cascata[id1].at(r) + (4+flag3)*sistema_a->hidreletricasVtr.size() + JJ + jj + j + delta] = ptr_x_subp[id1][(4+flag3)*cascata[id1].size() + j + jjj + JJJ + delta_subP];		//q
					ptr_x_subp_agg[0][cascata[id1].at(r) + (4+flag3)*sistema_a->hidreletricasVtr.size() + 2*JJ + jj + j + delta] = ptr_x_subp[id1][(4+flag3)*cascata[id1].size() + j + jjj + 2*JJJ + delta_subP];	//z
				}
				jjj += sistema_a->hidreletricasVtr[cascata[id1].at(r)].GetNGrupos();
			}
			if (((t + 1 - sistema_a->GetTt1())%(sistema_a->GetTt2() - sistema_a->GetTt1()) == 0) && ((t + 1 - sistema_a->GetTt1()) > 0))
		...
		if (flag2 == 1)
		{
			...
			{
				for (size_t r = 0; r < cascata[id1].size(); r++)
					ptr_x_subp_agg[0][delta + cascata[id1].at(r)] = ptr_x_subp[id1][delta_subP + r];		//vfol
				delta += nt * (sistema_a->GetTt2() - sistema_a->GetTt1()) + sistema_a->hidreletricasVtr.size();
				delta_subP += nt_subP * (sistema_a->GetTt2() - sistema_a->GetTt1()) + cascata[id1].size();
			}
		}
	}
	for (int comp = CH; comp < CH + I; comp++)		//Termeletrico
	{
	...
	for (int comp = CH + I; comp < CH + I + N; comp++)		//Demanda
	{
		...
		for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
		{
			...
			if (sistema_a->GetFlagPhmax() == 1)
				ptr_x_subp_agg[0][n + r + sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size() + deltaa] = ptr_x_subp[comp][r + sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1) + sistema_a->hidreletricasVtr.size()];		//phmax
		}
	}
}
void Spcdec2Results::AlocarXeXa()
{
	...
	for (int comp = 0; comp < CH; comp++)	//Hidreletrico
	{
		...
		for (int t = 0; t < N; t++)
		{
			...
			for (size_t r = 0; r < cascata[id1].size(); r++)
			{
				x[cascata[id1].at(r) + delta] = ptr_x_subp[id1][r + delta_subP];												//ph
				x[cascata[id1].at(r) + sistema_a->hidreletricasVtr.size() + delta] = ptr_x_subp[id1][r + cascata[id1].size() + delta_subP];			//v
				x[cascata[id1].at(r) + 2*sistema_a->hidreletricasVtr.size() + delta] = ptr_x_subp[id1][r + 2*cascata[id1].size() + delta_subP];		//d
				x[cascata[id1].at(r) + 3*sistema_a->hidreletricasVtr.size() + delta] = ptr_x_subp[id1][r + 3*cascata[id1].size() + delta_subP];		//s
				if (sistema_a->GetFlagPhmax() == 1)
					x[cascata[id1].at(r) + (4+flag3)*sistema_a->hidreletricasVtr.size() + delta] = ptr_x_subp[id1][r + 4*cascata[id1].size() + delta_subP];		//phmax
				...
				for (int j = 0; j < sistema_a->hidreletricasVtr[cascata[id1].at(r)].GetNGrupos(); j++)
				{
					x[cascata[id1].at(r) + (4+flag3)*sistema_a->hidreletricasVtr.size() + jj + j + delta] = ptr_x_subp[id1][(4+flag3)*cascata[id1].size() + j + jjj + delta_subP];					//phg
					x[cascata[id1].at(r) + (4+flag3)*sistema_a->hidreletricasVtr.size() + JJ + jj + j + delta] = ptr_x_subp[id1][(4+flag3)*cascata[id1].size() + j + jjj + JJJ + delta_subP];		//q
					x[cascata[id1].at(r) + (4+flag3)*sistema_a->hidreletricasVtr.size() + 2*JJ + jj + j + delta] = ptr_x_subp[id1][(4+flag3)*cascata[id1].size() + j + jjj + 2*JJJ + delta_subP];	//z
				}
				jjj += sistema_a->hidreletricasVtr[cascata[id1].at(r)].GetNGrupos();
			}
			if (((t + 1 - sistema_a->GetTt1())%(sistema_a->GetTt2() - sistema_a->GetTt1()) == 0) && ((t + 1 - sistema_a->GetTt1()) > 0))
		...
		if (flag2 == 1)
		{
			...
			{
				for (size_t r = 0; r < cascata[id1].size(); r++)
					x[delta + cascata[id1].at(r)] = ptr_x_subp[id1][delta_subP + r];		//vfol
				delta += nt * (sistema_a->GetTt2() - sistema_a->GetTt1()) + sistema_a->hidreletricasVtr.size();
				delta_subP += nt_subP * (sistema_a->GetTt2() - sistema_a->GetTt1()) + cascata[id1].size();
			}
		}
	...
	for (int comp = CH + I; comp < CH + I + N; comp++)		//Demanda
	{
		...
		for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
		{
			...
			if (sistema_a->GetFlagPhmax() == 1)
				xa[r + sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size() + deltaa] = ptr_x_subp[comp][r + sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1) + sistema_a->hidreletricasVtr.size()];		//phmax
		}
	}
}

void Spcdec2Results::ExportarX(string nome_arquivo)
{
...
void Spcdec2Results::ExportarXA(string nome_arquivo)
{
	inFile = new ofstream( nome_arquivo, ios::out );
	if ( inFile->is_open() )
	{
...
void Spcdec2Results::ExportarXmed(string nome_arquivo)
{
	inFile = new ofstream( nome_arquivo, ios::out );
	if ( inFile->is_open() )
	{
...
void Spcdec2Results::ZerarSolucoes()
{
	x.clear();
	x_med.clear();
	xa.clear();
...
void Spcdec2Results::CalcularNormaSubgXtil(vetorfloat &norma1, vetorfloat &norma2, vetorfloat &normaInf)
{
	// INCOMPLETO, FALTA FAZER NORMAINF
	...
	for (int t = 0; t < N; t++)
	{
		if (sistema_a->GetFlagTbinaryModel() == 0)
			for (size_t i = 0; i < sistema_a->termeletricasVtr.size(); i++)
			{
				dif = - ptr_x_til[t + CH + I][i] + ptr_x_til[i + CH][t*(3 + flag4)];		// - pta + pt
				norma1[0] += abs(dif);
				norma2[0] += pow(dif, 2);
			}
		else
			for (size_t i = 0; i < sistema_a->termeletricasVtr.size(); i++)
			{
				dif = - ptr_x_til[t + CH + I][i] + ptr_x_til[i + CH][t*(3 + flag4 + flag7)] + ptr_x_til[i + CH][t*(3 + flag4 + flag7) + 1]*sistema_a->termeletricasVtr[i].GetPmin();		//- pta + pt + u*pt_min
				norma1[0] += abs(dif);
				norma2[0] += pow(dif, 2);
			}
		deltaa += sistema_a->termeletricasVtr.size();
		...
		for (size_t ch = 0; ch < cascata.size(); ch++)
		{
			...
			for (size_t r = 0; r < cascata[ch].size(); r++)
			{
				dif = - ptr_x_til[t + CH + I][cascata[ch].at(r) + sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1)] + ptr_x_til[ch][r + delta];		// - pha + ph
				norma1[1] += abs(dif);
				norma2[1] += pow(dif, 2);
				if (sistema_a->GetFlagPhmax() == 1)
				{
					dif = - ptr_x_til[t + CH + I][cascata[ch].at(r) + sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1) + sistema_a->hidreletricasVtr.size()] + ptr_x_til[ch][r + 4*cascata[ch].size() + delta];		// - phmaxa + phmax
					norma1[2] += abs(dif);
					norma2[2] += pow(dif, 2);
				}
				else
				{
					for (int j = 0; j < sistema_a->hidreletricasVtr[cascata[ch].at(r)].GetNGrupos(); j++)
						som += ptr_x_til[ch][jj + j]*sistema_a->hidreletricasVtr[cascata[ch].at(r)].grupoVtr[j].GetPmax()*sistema_a->hidreletricasVtr[cascata[ch].at(r)].grupoVtr[j].GetNUnidades();		// z
					jj += sistema_a->hidreletricasVtr[cascata[ch].at(r)].GetNGrupos();
					som += - ptr_x_til[ch][r + delta];
				}
			}
		}
		if (sistema_a->GetFlagPhmax() == 0)
...
void Spcdec2Results::CalcularNormaSubgXtil(vetorfloat &normas)
{
	...
	if ( Aggrgtd )
	{
		...
		for (int t = 0; t < N; t++)
		{
			...
			for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
			{
				...
				if (sistema_a->GetFlagPhmax() == 1)
				{
					dif = - ptr_x_til[0][n + r + R + delta_sg] + ptr_x_til[0][r + 4*R + delta];		//phmax
					normas[0] += abs(dif);
					normas[1] += pow(dif, 2);
					if (normas[2] < abs(dif))
						normas[2] = abs(dif);
...
void Spcdec2Results::AlocarXmed(CMatrizEsparsa * x_spr)
{
	// Alocar x_med
	for  (int i = 0; i < n; i++)
	...
	for (int t = 0; t < N; t++)
	{
		...
		for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
		{
			...
			if (sistema_a->GetFlagPhmax() == 1)
				x_spr->SubstituirElemento(r + (4+flag3)*sistema_a->hidreletricasVtr.size() + delta, 0, (x[r + (4+flag3)*sistema_a->hidreletricasVtr.size() + delta] + xa[r + sistema_a->hidreletricasVtr.size() + deltaa]) / 2);	//phmax
		}
		delta -= (3+flag7+flag4)*sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1);
		deltaa -= sistema_a->termeletricasVtr.size();
		if (((t + 1 - sistema_a->GetTt1())%(sistema_a->GetTt2() - sistema_a->GetTt1()) == 0) && ((t + 1 - sistema_a->GetTt1()) > 0))
