...
/*--------------------------------------------------------------------------*/
/** @file
 * Scndec2Fi is a "concrete" class which implements the
 * interface defined by the abstract base class FiOracle. Used to solve the
 * Scenario Decomposition scheme 2
 *
 * \version 0.50
 *
 * \date 16 - 05 - 2012
...
Scndec2Fi::Scndec2Fi( CSistema * const sistema_end , Scndec2Results * const resultadosGurobi_end, int tipo_dual)
	:
        FiOracle(), ambGRB(GRBEnv())
{
	...
	na = sistema_a->GetTt1() * (2*sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size() + JJ);
...
void Scndec2Fi::SetData( cLMRow UB  )
{
	UpBnd = UB;    
 }

void Scndec2Fi::CriarModelos( void )
{
	// cenário 0
	vtr_subproblemaC.push_back(new Scndec2SPC(sistema_a, ambGRB));

	// demais cenários
	for (int n_cen = 1; n_cen < sistema_a->GetNCenarios(); n_cen++)
		vtr_subproblemaC.push_back(new Scndec2SPC(sistema_a, n_cen, *vtr_subproblemaC[0]));
...
bool Scndec2Fi::GetUC( cIndex i )
{
	return( true );
}

LMNum Scndec2Fi::GetUB( cIndex i )
{
	return( Inf<LMNum>() );
}
...
void Scndec2Fi::SetLambda( cLMRow Lmbd )
{
	Lam1 = Lmbd;
...
void Scndec2Fi::SetLamBase( cIndex_Set LmbdB , cIndex LmbdBD )
{
	LamB = LmbdB;
	LamBd = LmbdBD;
}

bool Scndec2Fi::SetPrecision( HpNum Eps )
{
...
HpNum Scndec2Fi::Fi( cIndex wFi )
{
	...
	{
		throw NDOException( "Scndec2Fi: Fi called fon an easy component." );	// Essa funçao nao pode ser chamada para easy components
		return( - Inf<HpNum>() );
	}
...
bool Scndec2Fi::NewGi( cIndex wFi )
{
	last_wFi = wFi;
	if( wFi == 0 )
...
Index Scndec2Fi::GetGi( SgRow SubG , cIndex_Set &SGBse , cIndex Name , cIndex strt , Index stp )
{
	// timer on - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	if( Fit )
...
HpNum Scndec2Fi::GetVal( cIndex Name )
{
	if ( Name < MBN )
...
void Scndec2Fi::SetGiName( cIndex Name )
{
...
HpNum Scndec2Fi::GetLowerBound( cIndex wFi )
{
...
FiOracle::FiStatus Scndec2Fi::GetFiStatus( Index wFi )
{
	...
	if (wFi > GetNrFi())
	{
		...
		if ( log_auxiliar->is_open() )
		{
			...
			*log_auxiliar << char(9) << norma_compl[0];
		...
		if (iter_count == 1)
		{
			...
			//Slvr->SetPar(Bundle::ktStar, 50*tinicial);		// altera tStar mas no log fica o valor inicial

			//Slvr->SetPar(Bundle::ktCurr, tinicial);
...
void Scndec2Fi::Deleted( cIndex Name )
{
	ptr_x_names[Name].clear();
...
void Scndec2Fi::Aggregate( cHpRow Mlt , cIndex_Set NmSt , cIndex Dm , cIndex NwNm )
{
...
HpNum Scndec2Fi::GetHeuristicSolution(  )
{
	return 0;
	//return heuristica->GetFO();
}

vetorfloat Scndec2Fi::GenPreconditioners()
{
