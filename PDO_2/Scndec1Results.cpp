#include "Scndec1Results.h"

Scndec1Results::Scndec1Results(CSistema * const sistema_end)
{
	...
	if (flag3 == 1)
		cout << "Scndec1Results: classe não preparada para modelo com variavel phmax! Desnecessario." << endl;
	...
	nd = sistema_a->GetTt1() * ((3+flag4+flag7)*sistema_a->termeletricasVtr.size() + sistema_a->barrasVtr.size() - 1 + 4*sistema_a->hidreletricasVtr.size() + 3*JJ + sistema_a->barrasVtr.size());
	if ( flag1 == 0 )		// Remover teta, def e deixar uma variável de deficit por período
	{
		...
		nd -= sistema_a->GetTt1() * (sistema_a->barrasVtr.size() - 1 + sistema_a->barrasVtr.size() - 1);
	...
	else if ( flag1 >= 2)	// Remover somente teta
	{
		...
		nd -= sistema_a->GetTt1() * (sistema_a->barrasVtr.size() - 1);
...
Scndec1Results::~Scndec1Results(void)
{
	delete inFile;
}

void Scndec1Results::AlocarXmed(CMatrizEsparsa * x_spr)
{
...
// Lambda:
//L_cen = [Lpt Lu Lcp (LF) (Lteta) Lph Lv Ld Ls (Lphmax) Lphg Lq Lz (Ldef)] -> (.) tamanho dos vetores que dependem de flags
//L_cen = [Lpt Lu Lup Lud (LF) (Lteta) Lph Lv Ld Ls (Lphmax) Lphg Lq Lz (Ldef)] -> (.) tamanho dos vetores que dependem de flags

void Scndec1Results::SetComponents( int * comp_inf, bool aggr )
{
	//Aggrgtd = aggr;
...
void Scndec1Results::GravarSolucao(double obj_a, vetorfloat x_a, int status_a, int comp, double lb_a)
{
...
void Scndec1Results::GetSubGradiente(int wFi_a, double * SubG)
{
	...
	// ndt vai até o número de variáveis de primeiro estágio

	for (int n_cen = 0; n_cen < sistema_a->GetNCenarios() - 1; n_cen++)
	{
		for (int i = 0; i < ndt; i++)		// loop nas variáveis de primeiro estágio
		{
			if ((wFi_a - 1) == n_cen)		// subgradiente referente ao multiplicador da componente (wFi_a - 1)
				SubG[i + ndt*n_cen] = (1 - sistema_a->hidreletricasVtr[0].GetProbAfluencia(wFi_a - 1) ) * ptr_x_subp[wFi_a - 1][i];
			else							// subgradiente referente aos multiplicadores das demais componentes
				SubG[i + ndt*n_cen] = - sistema_a->hidreletricasVtr[0].GetProbAfluencia(wFi_a - 1) * ptr_x_subp[wFi_a - 1][i];
		}
	}
	for (int i = 0; i < nd; i++)
		SubG[i] *= sistema_a->GetPrecondidioner(i);
}
void Scndec1Results::GetSubGradiente(double * SubG)
{
	int ndt = (nd / (sistema_a->GetNCenarios() - 1));		// numero de var. duais por cenário, diferente do numero de variáveis por subproblema!
	...
	for (int i = 0; i < ndt; i++)
	{
		...
		for (int n_cen = 0; n_cen < sistema_a->GetNCenarios(); n_cen++)
			valor_esperado[i] += sistema_a->hidreletricasVtr[0].GetProbAfluencia(n_cen) * ptr_x_subp[n_cen][i];
	}
	for (int n_cen = 0; n_cen < sistema_a->GetNCenarios() - 1; n_cen++)
		for (int i = 0; i < ndt; i++)		// loop nas variáveis de primeiro estágio
			SubG[i + ndt*n_cen] = ptr_x_subp[n_cen][i] - valor_esperado[i];
	
	for (int i = 0; i < nd; i++)
		SubG[i] *= sistema_a->GetPrecondidioner(i);
}
void Scndec1Results::GetSubGradiente(int wFi_a, double * SubG, vetorfloat x_name)
{
	...
	if (wFi_a > sistema_a->GetNCenarios())		// agregado
	{
		...
		for (int i = 0; i < ndt; i++)
		{
			valor_esperado.push_back(0);
			for (int n_cen = 0; n_cen < sistema_a->GetNCenarios(); n_cen++)
				valor_esperado[i] += sistema_a->hidreletricasVtr[0].GetProbAfluencia(n_cen) * ptr_x_name_subp[n_cen][i];
		}
		for (int n_cen = 0; n_cen < sistema_a->GetNCenarios() - 1; n_cen++)
			for (int i = 0; i < ndt; i++)		// loop nas variáveis de primeiro estágio
				SubG[i + ndt*n_cen] = ptr_x_name_subp[n_cen][i] - valor_esperado[i];
	}
	else		// desagregado
	{
		for (int n_cen = 0; n_cen < sistema_a->GetNCenarios() - 1; n_cen++)
		{
			...
			{
				if ((wFi_a - 1) == n_cen)		// subgradiente referente ao multiplicador da componente (wFi_a - 1)
					SubG[i + ndt*n_cen] = (1 - sistema_a->hidreletricasVtr[0].GetProbAfluencia(wFi_a - 1) ) * x_name[i];
				else							// subgradiente referente aos multiplicadores das demais componentes
					SubG[i + ndt*n_cen] = - sistema_a->hidreletricasVtr[0].GetProbAfluencia(wFi_a - 1) * x_name[i];
			}
		}
	}
	for (int i = 0; i < nd; i++)
...
//
vetorfloat Scndec1Results::GetCompX(int comp)
{
	if ( Aggrgtd )
	{
...
void Scndec1Results::AlocarX_med()
{
	// Alocar x_med a partir de ptr_x_subp
	int ndt = (nd / sistema_a->GetNCenarios());		// numero de var. duais por cenário, diferente do numero de variáveis por subproblema!
	...
	for (int n_cen = 0; n_cen < sistema_a->GetNCenarios(); n_cen++)
		for (int i = 0; i < n_var_2_est; i++)
			x_med[ndt + i + n_var_2_est*n_cen] = ptr_x_subp[n_cen][ndt + i];
}
vetorfloat Scndec1Results::GetX_med(bool til)
{
...
void Scndec1Results::AdicionarLBXtil(CMatrizEsparsa * x_spr, double mult)
{
...
void Scndec1Results::ExportarXmed(string nome_arquivo)
{
...
void Scndec1Results::ZerarSolucoes()
{
	x_med.clear();
	obj_subp.clear();
	ptr_x_subp.clear();
...
void Scndec1Results::CalcularNormaSubgXtil(vetorfloat &norma)
{
...
void Scndec1Results::CalcularNormaSubgXtilTV(vetorfloat &norma)
{
