...
CMatrizEsparsa Spcdec2SPD::MatrizRestDemanda(int n_a, int no)	// Monta matriz da restrição de atendimento a demanda
{
...
CMatrizEsparsa Spcdec2SPD::MatrizRestDemandaBarraUnica(int n_a, int no)	// Monta matriz da restrição de atendimento a demanda
{
	int flag3 = int (sistema_a->GetFlagPhmax());
...
CMatrizEsparsa Spcdec2SPD::MatrizLimFluxo(int n_a, int no)
{
...
CMatrizEsparsa Spcdec2SPD::MatrizReserva(int n_a, int no)
{
	int flag1 = sistema_a->GetFlagModeloRede();
...
vetorfloat2 Spcdec2SPD::LimRestDemanda(int n_a, int no)
{
	int cen = 0;
	int t = 0;
...
vetorfloat2 Spcdec2SPD::LimRestDemandaBarraUnica(int n_a, int no)
{
	int cen = 0;
	int t = 0;
	if (no < sistema_a->GetTt1())		// identifica o cenário e o período de tempo do nó
...
vetorfloat2 Spcdec2SPD::LimFluxo0(int n_a, int no)
{
...
vetorfloat2 Spcdec2SPD::LimFluxo2(int n_a, int no)
{
	int L = sistema_a->linhasVtr.size();
	vetorfloat2 Lim;
	DimensionarMatriz(&Lim, L, 2);
...
vetorfloat2 Spcdec2SPD::LimReserva(int n_a, int no)
{
	//int cen = 0;
	//int t = 0;
	//if (no < sistema_a->GetTt1())		// identifica o cenário e o período de tempo do nó
...
void Spcdec2SPD::MatrizRestricoesLineares(int n_a, CMatrizEsparsa &MM, int no)		// Matriz M é composta de 3 vetores, cada coluna (vetor) é : [indexL indexC Valor]
{
	CMatrizEsparsa M(0);
	if ( sistema_a->GetFlagModeloRede() == 1 )
	{
...
void Spcdec2SPD::MatrizLimitesLineares(int n_a, vetorint * LimTipo, vetorfloat * LimValor, int no)
{
	vetorfloat2 L;
	if ( sistema_a->GetFlagModeloRede() == 1 )
	{
...
Spcdec2SPD::Spcdec2SPD(CSistema * const sistema_end, const GRBEnv &ambiente_gurobi)
{
...
void Spcdec2SPD::CriarVariaveis(int no)
{
	try 
	{
...
void Spcdec2SPD::CriarRestricoes(int no)
{
	try
	{
		vetorint LimTipo;
...
void Spcdec2SPD::CriarFuncaoObjetivoRL(const vetorfloat * const lambda, int no)
{
	try 
	{
		int cen = 0;
		...
		for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)
		{
			...
			if (sistema_a->GetFlagPhmax() == 1)
				vars[no][r + sistema_a->termeletricasVtr.size() + flag1a*(sistema_a->barrasVtr.size() - 1) + sistema_a->hidreletricasVtr.size()].set(GRB_DoubleAttr_Obj, lambda->at(r + sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size() + delta_dual));		//phmax
		}
...
int Spcdec2SPD::ResolverProblemaRL(Spcdec2Results * resultadoGurobi, const vetorfloat * const lambda, const int iter, int no)
{
	...
	try
	{
		...
		resultadoGurobi->GravarSolucao(fo_n, x_n, nStatus, resultadoGurobi->GetCH() + resultadoGurobi->GetI() + no);
	} catch(GRBException e) {
		cout << "Error code = " << e.getErrorCode() << endl;
		cout << e.getMessage() << endl;
		resultadoGurobi->GravarSolucao(fo_n, x_n, e.getErrorCode(), resultadoGurobi->GetCH() + resultadoGurobi->GetI() + no);
	} catch(...) {
		cout << "Exception during optimization" << endl;
	}
	return 0;
...
Spcdec2SPD::Spcdec2SPD(CSistema * const sistema_end)
{
	sistema_a = sistema_end;
	int flag3 = int (sistema_a->GetFlagPhmax());
	n_nos = sistema_a->GetTt1() + (sistema_a->GetTt2() - sistema_a->GetTt1()) * sistema_a->GetNCenarios();
...
void Spcdec2SPD::GerarRestricoes(int no)
{
...
void Spcdec2SPD::GerarVarBounds(int no, double *lbd , double *ubd)
{
	// variáveis em cada problema somente para um nó
	int cen = 0;
	int t = 0;
...
void Spcdec2SPD::GerarCoefFuncaoObjetivo(int no, double *cst)
{
	int cen = 0;
	int t = 0;
	if (no < sistema_a->GetTt1())		// identifica o cenário e o período de tempo do nó
...
void Spcdec2SPD::GetBDesc( int comp , int *Bbeg , int *Bind , double *Bval , double *lhs , double *rhs , double *cst , double *lbd , double *ubd )
{
...
int Spcdec2SPD::GetANZ(int comp)
{
...
void Spcdec2SPD::GetADesc( int comp , int *Abeg , int *Aind , double *Aval )
{
	// Termos da RL
	// lambda [pt ph v d (phmax)]
	// x = [pt teta ph phmax def]
	int flag3 = int (sistema_a->GetFlagPhmax());
	int nt_dual = sistema_a->termeletricasVtr.size() + (1 + flag3) * sistema_a->hidreletricasVtr.size() + (1 - flag3);
	int T = sistema_a->GetTt1() + sistema_a->GetNCenarios() * (sistema_a->GetTt2() - sistema_a->GetTt1());
	int ind = 0;
	int iAbeg = 0;
	Abeg[iAbeg++] = 0;
	...
	if (sistema_a->GetFlagPhmax() == 1)
		for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)		// phmax
		{
			Aind[ind] = r + sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size() + delta_dual;
			//Aval[ind++] = 1;
			Aval[ind++] = sistema_a->GetPrecondidioner(r + sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size() + delta_dual);
			Abeg[iAbeg++] = ind;
		}
