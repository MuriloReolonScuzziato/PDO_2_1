...
CMatrizEsparsa Scndec2Results::MatrizCalcFluxo(int n_a)
{
	...
	CMatrizEsparsa Alin(int (T * sistema_a->linhasVtr.size()), n_a);
	...
	for ( int t = 0; t < T; t++)
	{
		...
		if (((t + 1 - sistema_a->GetTt1())%(sistema_a->GetTt2() - sistema_a->GetTt1()) == 0) && ((t + 1 - sistema_a->GetTt1()) > 0))
			c += (n_a / T) + int(flag2*sistema_a->hidreletricasVtr.size());
		else
			c += (n_a / T);
	}
	return Alin;
}
CMatrizEsparsa Scndec2Results::MatrizCalcFluxo_cen(int n_a)
{
	CMatrizEsparsa Alb(int(sistema_a->linhasVtr.size()), int(sistema_a->barrasVtr.size()));
	for ( size_t l = 0; l < sistema_a->linhasVtr.size(); l++)
		for ( size_t b = 0; b < sistema_a->barrasVtr.size(); b++)
...
Scndec2Results::Scndec2Results(CSistema * const sistema_end)
{
	...
	if (flag3 == 1)
		cout << "Scndec2Results: classe não preparada para modelo com variavel phmax! Desnecessario." << endl;
	...
	nd = sistema_a->GetTt1() * (2*sistema_a->termeletricasVtr.size() + sistema_a->hidreletricasVtr.size() + JJ);
...
Scndec2Results::~Scndec2Results(void)
{
	delete inFile;
}
...
// Lambda:
//L_cen = [Lpt Lu Ld Lphg] -> (.) tamanho dos vetores que dependem de flags

void Scndec2Results::SetComponents( int * comp_inf, bool aggr )
{
	//Aggrgtd = aggr;
...
void Scndec2Results::GravarSolucao(double obj_a, vetorfloat x_a, int status_a, int comp, double lb_a)
{
...
void Scndec2Results::GetSubGradiente(int wFi_a, double * SubG)
{
	...
	for (int n_cen = 0; n_cen < sistema_a->GetNCenarios() - 1; n_cen++)
	{
		...
		if ((wFi_a - 1) == n_cen)		// subgradiente referente ao multiplicador da componente (wFi_a - 1)
		{
			for (int t = 0; t < sistema_a->GetTt1(); t++)
			{
				...
				{
					for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)	
						SubG[j + deltasg + ndt*n_cen] = (1 - sistema_a->hidreletricasVtr[0].GetProbAfluencia(wFi_a - 1) ) * ptr_x_subp[wFi_a - 1][j + deltax];
					deltax += sistema_a->hidreletricasVtr[r].GetNGrupos();
					deltasg += sistema_a->hidreletricasVtr[r].GetNGrupos();
				}
		...
		else							// subgradiente referente aos multiplicadores das demais componentes
		{
			for (int t = 0; t < sistema_a->GetTt1(); t++)
			{
				...
				{
					for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)	
						SubG[j + deltasg + ndt*n_cen] = - sistema_a->hidreletricasVtr[0].GetProbAfluencia(wFi_a - 1) * ptr_x_subp[wFi_a - 1][j + deltax];
					deltax += sistema_a->hidreletricasVtr[r].GetNGrupos();
					deltasg += sistema_a->hidreletricasVtr[r].GetNGrupos();
				}
				for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//q, z
...
void Scndec2Results::GetSubGradiente(double * SubG)
{
	int ndt = (nd / (sistema_a->GetNCenarios() - 1));		// numero de var. duais por cenário, diferente do numero de variáveis por subproblema!
	...
	for (int t = 0; t < sistema_a->GetTt1(); t++)
	{
		...
		for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//phg
		{
			for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)	
			{
				valor_esperado.push_back(0);
				for (int n_cen = 0; n_cen < sistema_a->GetNCenarios(); n_cen++)
					valor_esperado[j + deltasg] += sistema_a->hidreletricasVtr[0].GetProbAfluencia(n_cen) * ptr_x_subp[n_cen][j + deltax];
			}
			deltax += sistema_a->hidreletricasVtr[r].GetNGrupos();
			deltasg += sistema_a->hidreletricasVtr[r].GetNGrupos();
		}
	...
	for (int n_cen = 0; n_cen < sistema_a->GetNCenarios() - 1; n_cen++)
	{
		...
		for (int t = 0; t < sistema_a->GetTt1(); t++)
		{
			...
			{
				for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)	
					SubG[j + deltasg + ndt*n_cen] = ptr_x_subp[n_cen][j + deltax] - valor_esperado[j + deltasg];
				deltax += sistema_a->hidreletricasVtr[r].GetNGrupos();
				deltasg += sistema_a->hidreletricasVtr[r].GetNGrupos();
			}
			for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//q, z
...
void Scndec2Results::GetSubGradiente(int wFi_a, double * SubG, vetorfloat x_name)
{
	...
	if (wFi_a > sistema_a->GetNCenarios())		// agregado
	{
		...
		for (int t = 0; t < sistema_a->GetTt1(); t++)
		{
			...
			for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//phg
			{
				for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)	
				{
					valor_esperado.push_back(0);
					for (int n_cen = 0; n_cen < sistema_a->GetNCenarios(); n_cen++)
						valor_esperado[j + deltasg] += sistema_a->hidreletricasVtr[0].GetProbAfluencia(n_cen) * ptr_x_name_subp[n_cen][j + deltax];
				}
				deltax += sistema_a->hidreletricasVtr[r].GetNGrupos();
				deltasg += sistema_a->hidreletricasVtr[r].GetNGrupos();
			}
		...
		for (int n_cen = 0; n_cen < sistema_a->GetNCenarios() - 1; n_cen++)
		{
			...
			for (int t = 0; t < sistema_a->GetTt1(); t++)
			{
				...
				{
					for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)	
						SubG[j + deltasg + ndt*n_cen] = ptr_x_name_subp[n_cen][j + deltax] - valor_esperado[j + deltasg];
					deltax += sistema_a->hidreletricasVtr[r].GetNGrupos();
					deltasg += sistema_a->hidreletricasVtr[r].GetNGrupos();
				}
				for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//q, z
		...
		for (int n_cen = 0; n_cen < sistema_a->GetNCenarios() - 1; n_cen++)
		{
			...
			if ((wFi_a - 1) == n_cen)		// subgradiente referente ao multiplicador da componente (wFi_a - 1)
			{
				for (int t = 0; t < sistema_a->GetTt1(); t++)
				{
					...
					{
						for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)	
							SubG[j + deltasg + ndt*n_cen] = (1 - sistema_a->hidreletricasVtr[0].GetProbAfluencia(wFi_a - 1) ) * x_name[j + deltax];
						deltax += sistema_a->hidreletricasVtr[r].GetNGrupos();
						deltasg += sistema_a->hidreletricasVtr[r].GetNGrupos();
					}
					for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//q, z
			...
			else							// subgradiente referente aos multiplicadores das demais componentes
			{
				for (int t = 0; t < sistema_a->GetTt1(); t++)
				{
					...
					{
						for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)	
							SubG[j + deltasg + ndt*n_cen] = - sistema_a->hidreletricasVtr[0].GetProbAfluencia(wFi_a - 1) * x_name[j + deltax];
						deltax += sistema_a->hidreletricasVtr[r].GetNGrupos();
						deltasg += sistema_a->hidreletricasVtr[r].GetNGrupos();
					}
					for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//q, z
...
//
vetorfloat Scndec2Results::GetCompX(int comp)
{
	if ( Aggrgtd )
	{
...
void Scndec2Results::AlocarX_med()
{
...
vetorfloat Scndec2Results::GetX_med(bool hat_or_til)
{
...
void Scndec2Results::ExportarXmed(string nome_arquivo)
{
...
void Scndec2Results::ZerarSolucoes()
{
	x_med.clear();
	obj_subp.clear();
	ptr_x_subp.clear();
...
void Scndec2Results::CalcularNormaSubgXtil(vetorfloat &norma)
{
		...
		for (int t = 0; t < sistema_a->GetTt1(); t++)
		{
			...
			for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//phg
			{
				for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)	
				{
					valor_esperado.push_back(0);
					for (int n_cen = 0; n_cen < sistema_a->GetNCenarios(); n_cen++)
						valor_esperado[j + deltasg] += sistema_a->hidreletricasVtr[0].GetProbAfluencia(n_cen) * ptr_x_til[0][j + deltax + n_sub*n_cen];
				}
				deltax += sistema_a->hidreletricasVtr[r].GetNGrupos();
				deltasg += sistema_a->hidreletricasVtr[r].GetNGrupos();
			}
		...
		for (int n_cen = 0; n_cen < sistema_a->GetNCenarios() - 1; n_cen++)
		{
			...
			for (int t = 0; t < sistema_a->GetTt1(); t++)
			{
				...
				for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//phg
				{
					for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)	
					{
						dif = ptr_x_til[0][j + deltax + n_sub*n_cen] - valor_esperado[j + deltasg];
						norma[0] += abs(dif);
						norma[1] += pow(dif, 2);
						if (norma[2] < abs(dif))
							norma[2] = abs(dif);
		...
		for (int t = 0; t < sistema_a->GetTt1(); t++)
		{
			...
			for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//phg
			{
				for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)	
				{
					valor_esperado.push_back(0);
					for (int n_cen = 0; n_cen < sistema_a->GetNCenarios(); n_cen++)
						valor_esperado[j + deltasg] += sistema_a->hidreletricasVtr[0].GetProbAfluencia(n_cen) * ptr_x_til[n_cen][j + deltax];
				}
				deltax += sistema_a->hidreletricasVtr[r].GetNGrupos();
				deltasg += sistema_a->hidreletricasVtr[r].GetNGrupos();
			}
		...
		for (int n_cen = 0; n_cen < sistema_a->GetNCenarios() - 1; n_cen++)
		{
			...
			for (int t = 0; t < sistema_a->GetTt1(); t++)
			{
				...
				for (size_t r = 0; r < sistema_a->hidreletricasVtr.size(); r++)	//phg
				{
					for (int j = 0; j < sistema_a->hidreletricasVtr[r].GetNGrupos(); j++)	
					{
						dif = ptr_x_til[n_cen][j + deltax] - valor_esperado[j + deltasg];
						norma[0] += abs(dif);
						norma[1] += pow(dif, 2);
						if (norma[2] < abs(dif))
							norma[2] = abs(dif);
...
void Scndec2Results::CalcularNormaSubgXtilTV(vetorfloat &norma)
{
